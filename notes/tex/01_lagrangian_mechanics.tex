...
concepts that the next chapter converts into Hamiltonian phase space. The point
is not to replace Newton's law by harder notation; it is to
make clear which parts of mechanics are coordinate choices and which parts are
geometric structure.

The primitive object here is a path in configuration space, and the
variational principle selects the physical path.  The phase-space viewpoint is
deliberately deferred to Chapter~\ref{chap:hamiltonian-phase-space}.  This
separation keeps the reader from treating momenta and symplectic forms as
automatic decorations of Lagrangian mechanics; they appear only after the
Legendre transform has been justified.
...
Constraints and symmetry are not side remarks. Holonomic constraints replace
\(Q\) by a constraint submanifold or introduce multipliers that measure
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[blue!70!black, fill=white, inner sep=1.5pt] at (4.7,0.43) {\(q(t)\)};
  \node[red!75!black, fill=white, inner sep=1.5pt] at (4.6,1.27) {\(s=e^{\gamma t/2}q\)};
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[fill=white, inner sep=1.5pt] at (2.85,2.45) {rolling hoop \(r\)};
\end{tikzpicture}
...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[x=0.72cm,y=1.0cm, >=Latex, font=\small]
  \draw[->] (0,0) -- (9.35,0) node[right] {\(r/R\)};
  ...
  \draw[red!75!black, thick, domain=1.12:8.9, samples=220]
    plot (\x,{0.45+1.25*(1-1/\x)*(1+5/(\x*\x))});
  \fill[red!75!black] (1.84,1.87) circle (1.3pt);
  \node[red!75!black, fill=white, inner sep=1.5pt] at (3.05,2.35)
    {unstable circular orbit};
  \fill[blue!70!black] (8.16,1.63) circle (1.3pt);
  \node[blue!70!black, fill=white, inner sep=1.5pt] at (7.10,1.20)
    {stable circular orbit};
  \draw[blue!70!black, thick, ->] (7.50,1.27) -- (8.07,1.58);
  \draw[red!75!black, thick, ->] (1.55,1.55) -- (1.78,1.82);
  \node[align=center] at (4.75,-0.75)
...
\end{itemize}
The point of listing them here is organizational: the same examples are first
derived from the action, then revisited in phase-space or reduced variables.
The computation should be read as a later layer of the same model, not as a
separate topic.
