...
\begin{tikzpicture}[scale=1.0, >=Latex]
  ...
  \fill (6.5,2.35) circle (2pt) node[above right, fill=white, inner sep=1pt] {\(q(t_1)\)};
...
Continuous symmetries of the action produce conserved quantities. Noether's
1918 theorem is the precise statement: every one-parameter group of symmetries
gives a conserved Noether charge. A symmetry is a transformation of the
configuration space that leaves the action unchanged, or changes it only by an
endpoint term. The infinitesimal version of that transformation is a vector
field \(\xi_Q\) along which the action has zero interior first variation. The
conserved quantity is the momentum paired with that vector field, corrected by
the endpoint term when one is present.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[fill=white, inner sep=1pt] (boundlabel) at (5.3,-1.23) {bound \(E<0\)};
  \draw[->, thin] (boundlabel.north west) -- (4.65,-0.7);
...
\(q(t)\in Q\), and an action \(S=\int L\,\dd t\). Stationarity gives the
Euler-Lagrange equations, and the endpoint term defines conjugate momentum.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \begin{scope}[xshift=5.0cm, xscale=0.58]
    ...
    \draw[blue!70!black, thick] (-2.95,1.22) -- (-2.55,1.22);
    \node[blue!70!black, align=left, fill=white, inner sep=1.5pt] at (-1.62,1.22)
      {\(\omega<\omega_c\): one minimum};
    \draw[red!75!black, thick] (-2.95,0.84) -- (-2.55,0.84);
    \node[red!75!black, align=left, fill=white, inner sep=1.5pt] at (-1.58,0.84)
...
plane, in units \(c=1\). Here the parameter is Schwarzschild coordinate time, so this is the reduced
