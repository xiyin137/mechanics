...
forms of action at fixed energy. In modern Lagrangian mechanics the action need
...
Let \(f(t)\) be continuous on \((t_0,t_1)\), and suppose
\[
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \fill (1.0,-1.1) circle (1.4pt) node[above right] {minimum};
...
\[
  ...
  \widehat{\dot\Sigma}+[\widehat\Omega,\widehat\Sigma],
\]
which is the stated formula under the hat isomorphism.
...
an accidental conserved Runge-Lenz vector organize the conic sections; the
...
\begin{tikzpicture}[x=0.82cm,y=1.0cm, >=Latex, font=\small]
  ...
  \node[red!75!black, fill=white, inner sep=1.5pt] at (6.1,1.24) {\(s=e^{\gamma t/2}q\)};
  \node[align=center] at (4.1,-1.85)
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \draw[->, red!75!black, very thick] (1.65,1.65) -- (2.25,2.25);
  \node[red!75!black, fill=white, inner sep=1pt] at (2.34,2.36) {\(N\)};
  \draw[->, black!70, thick] (1.65,1.65) -- (1.65,0.75)
    node[below] {\(mg\)};
  \draw[->, blue!70!black, thick] (2.1,1.65) arc (0:65:0.45);
  \node[blue!70!black, fill=white, inner sep=1pt] at (2.55,1.92) {\(\dot\phi\)};
  \node[align=center] at (0,-1.95)
    {loss of contact occurs when the\\normal force reaches \(N=0\)};
  \node[fill=white, inner sep=1.5pt] at (-1.1,-1.1) {fixed cylinder \(R\)};
  \node[fill=white, inner sep=1.5pt] at (2.82,2.60) {rolling hoop \(r\)};
\end{tikzpicture}
...
horizon at \(r=R\), restricted to the equatorial plane and written with an arbitrary
worldline parameter. We treat it here as a classical central-force model whose
...
\begin{tikzpicture}[x=0.62cm,y=1.25cm, >=Latex, font=\small]
  ...
  \node[below] at (1.0,-0.15) {\(R\)};
  \node[below] at (3.0,-0.15) {\(3R\)};
