...
matter of taste; it is the first physical idealization. A poor choice turns
constraints into clutter. A good choice lets the variational principle see the
geometry directly.
...
examples of the formalism. Third, it prepares the boundary momentum and energy
concepts that become phase-space coordinates in Hamiltonian mechanics. The
...
The primitive object here is a path in configuration space, and the variational
...
symplectic forms appear as consequences of the Legendre transform.
...
calling a quantity conserved before verifying the symmetry hypothesis.
...
Hamilton's principle, in Hamilton's 1834--1835 form, is a critical-point
principle for the time integral of \(L=T-V\). The historical phrase
``principle of least action'' is older and refers to related Maupertuis-Euler
forms of action at fixed energy, especially Maupertuis' 1744 statement and
Euler's variational development. In modern Lagrangian mechanics the action need
not be a minimum; it may be a saddle or a maximum for some classes of
variations. The catenoid soap film is the familiar variational counterexample:
a stationary surface need not be the minimizing surface once the boundary data
are large enough. The word stationary is therefore the accurate word: the first
variation vanishes.
\end{remark}
...
Under the standing smoothness assumptions on \(L\), \(q\), and the variation,
...
Lagrange-d'Alembert principle adds virtual work to Hamilton's principle. The
admissible virtual displacements are the same ones used in the variational
problem, and the external force does virtual work \(Q_i\eta^i\). Thus
\[
...
It is important that \(Q_i\) is a covector along the path, not necessarily a
...
particle in Cartesian coordinates \(g_{ij}\) is just \(m\delta_{ij}\). For a
pendulum, double pendulum, or rigid body written in angles, the same kinetic
energy becomes a curved metric on the configuration space. The Christoffel
symbols below are therefore inertial terms caused by coordinates on a curved
configuration space, not new forces.
...
the constraint surface. Lagrange's multiplier rule says that a covector
...
constant rank \(k\) on the constraint surface. Under this assumption the
constrained equations take the multiplier form
\[
  \frac{\dd}{\dd t}\frac{\partial L}{\partial \dot q^i}
  -
  \frac{\partial L}{\partial q^i}
...
large dividend. Noether's 1918 theorem gave the general language that turns
continuous symmetries of the action into conserved quantities. A symmetry is a
transformation of the configuration space that leaves the action unchanged, or
changes it only by an endpoint term. The infinitesimal version of that
transformation is a vector field \(\xi_Q\) along which the action has zero
interior first variation. The conserved quantity is the momentum paired with
that vector field, corrected by the endpoint term when one is present.
...
boost charge includes the corresponding boundary term.
...
\]
Removing the arbitrary boost parameter gives the conserved center-of-mass
quantity \(m(x-\dot x\,t)\). The Lagrangian was not invariant as a function,
but the action changed only by an endpoint term.
\end{example}
...
Kepler motion is the first serious payoff of cyclic coordinates, Noether's
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \draw[dashed] (0,-0.7) -- (6.2,-0.7)
    node[right, fill=white, inner sep=1pt] {bound \(E<0\)};
  \draw[dashed] (0,0.45) -- (6.2,0.45)
    node[right, fill=white, inner sep=1pt] {scattering \(E>0\)};
  \node[blue, fill=white, inner sep=1.5pt] at (4.45,-1.05)
    {\(V_{\mathrm{eff}}(\rho)\)};
  \draw[densely dotted] (1.0,-1.1) -- (1.0,-1.58);
  \fill (1.0,-1.1) circle (1.4pt)
    node[above right, fill=white, inner sep=1pt] {minimum};
  \fill (0.624,-0.7) circle (1.3pt);
  \fill (2.52,-0.7) circle (1.3pt);
  \draw[<->, blue!70!black] (0.624,-1.58) -- (2.52,-1.58);
  \node[blue!70!black, fill=white, inner sep=1.5pt] at (1.57,-1.86)
...
\begin{remark}[Bertrand's theorem and the Kepler accident]
...
\(\SO(4)\) symmetry of bound motion. Historically, the vector is associated
with Laplace's celestial-mechanics work; its modern symmetry role was clarified
by Pauli's 1926 hydrogen-atom calculation and Fock's 1935 momentum-sphere
formulation.
\end{remark}
...
Euler's 1758 rigid-body equations are the historical source of the reduced
...
This calculation should be read from configuration space upward. The rigid body
can be introduced either from the Lagrangian on \(\SO(3)\) or from the reduced
Lie-Poisson structure. Here we begin with the Lagrangian because the
configuration, tangent constraint, and boundary terms are visible before
reduction. The later reduced Hamiltonian picture is obtained by using the
symmetry already visible in this variational formulation.

\section{What To Take Forward}
\label{sec:lagrangian-take-forward}

The Lagrangian formalism begins with a configuration manifold \(Q\), a path
\(q(t)\in Q\), and an action functional
\[
  S[q]=\int_{t_0}^{t_1} L(q,\dot q,t)\,\dd t .
\]
The Euler-Lagrange equations are the stationarity condition for variations with
fixed endpoints. Their boundary term defines the conjugate momentum. Natural
Lagrangians make the geometric content explicit: the kinetic energy is a
Riemannian metric on \(Q\), the Christoffel symbols describe the inertial terms
produced by curvilinear coordinates, and forces enter through potentials or
through the Lagrange-d'Alembert principle.

Constraints and symmetry are structural ingredients. Holonomic constraints
replace \(Q\) by a constraint submanifold or introduce multipliers that measure
constraint forces. Continuous symmetries produce conserved momenta through
Noether's theorem. The Kepler problem illustrates how rotational symmetry and
a hidden conserved Runge-Lenz vector organize the conic sections; the
rigid body illustrates how a finite-dimensional system can already contain
nontrivial geometry, body frames, inertia tensors, and reduced equations.

The boundary term now points to the Hamiltonian formulation. The Legendre
transform converts boundary momentum into a phase-space coordinate, and the same
action principle becomes the symplectic Hamiltonian formalism.

\paragraph{Questions to carry forward.}
When does a boundary term become a new state variable? Which constraints should
be solved by coordinates, and which should be kept as multipliers? Which
symmetries survive after a change of variables?
...
precession. The same algebraic structure appears in the general-relativistic
perihelion precession calculation that explained Mercury's anomalous advance in
Einstein's 1915 analysis, although the present subsection uses only an
effective classical central-force model.
...
\begin{tikzpicture}[x=0.82cm,y=1.0cm, >=Latex, font=\small]
  ...
  \node[red!75!black, fill=white, inner sep=1.5pt] at (5.85,1.24) {\(s=e^{\gamma t/2}q\)};
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \begin{scope}
    ...
    \draw[->, blue!70!black] (1.17,-1.17) -- (1.95,-1.17)
  ...
  \begin{scope}[xshift=5.0cm, xscale=0.58]
    ...
    \draw[red!75!black, thick, domain=-3.1416:3.1416, samples=180]
      plot (\x,{-1.25*sin(\x r)*sin(\x r)-0.28*cos(\x r)+0.35});
    \node[blue!70!black, align=center, fill=white, inner sep=1.5pt] at (-2.30,1.15)
      {\(\omega<\omega_c\)\\one minimum};
    \node[red!75!black, align=center, fill=white, inner sep=1.5pt] at (2.25,1.15)
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \draw[black!60, densely dashed] (0.70,1.28) -- (1.28,0.70)
    node[below right, fill=white, inner sep=1pt] {tangent};
  \draw[->, red!75!black, very thick] (hoop) -- (2.25,2.25);
  \node[red!75!black, fill=white, inner sep=1pt] at (2.42,2.28) {\(N\)};
  \draw[->, black!70, thick] (hoop) -- (1.65,0.75)
    node[below] {\(mg\)};
  \draw[->, blue!70!black, thick] (2.1,1.65) arc (0:65:0.45);
  \node[blue!70!black, fill=white, inner sep=1pt] at (2.48,1.82) {\(\dot\phi\)};
...
\[
  ...
  \qquad r>R.
\]
This is the geodesic Lagrangian for the Schwarzschild metric (1916), with
horizon at \(r=R\), restricted to the equatorial plane, in units \(c=1\). Here
the parameter is Schwarzschild coordinate time, so this is the reduced
square-root Lagrangian for spatial motion rather than the reparametrization-
invariant proper-time action. We treat it here as a classical central-force model whose
effective-potential analysis already yields the correct innermost stable
...
is the conserved Beltrami quantity because this reduced Lagrangian has no
...
\begin{tikzpicture}[x=0.62cm,y=1.25cm, >=Latex, font=\small]
  ...
  \node[align=center, fill=white, inner sep=1.5pt] at (5.3,-0.54)
...
\(-1/r+4/r^2-4/r^3\), which has an inner maximum between \(3R/2\) and \(3R\)
and an outer visible minimum with \(r_0>3R\).}
