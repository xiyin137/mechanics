...
derives the equations in detail.  This appendix separates the logical roles
that are easiest to blur: what is kinematics, what is balance law, what is
constitutive law, which stress measure is being used, what is a regularity
hypothesis, and where physical idealizations enter.  The goal is to prevent
correct formulas from being read with the wrong meaning.
...
This appendix uses \(X\) for material coordinates, \(x\) for spatial
coordinates, \(F\) for deformation gradient, \(\sigma\) for Cauchy stress,
\(P\) for first Piola-Kirchhoff stress, and \(D\) for the symmetric
strain-rate tensor in fluids.  The same letters may have had narrower meanings
in earlier chapters; the continuum meaning is restated where needed.
...
\section{Navier-Stokes Model Ingredients}
\label{sec:navier-stokes-model-ingredients}
...
\[
        ...
        \divergence v=0,
\]
combines the following ingredients:
\begin{enumerate}
\item a continuum approximation, with fields smooth enough on the length scales
of interest;
\item material parameters specialized to constant density \(\rho\) and viscosity
\(\mu\);
\item a constitutive assumption: Newtonian, isotropic, frame-indifferent
viscous stress;
\item the incompressibility constraint \(\divergence v=0\);
\item boundary and initial data compatible with that constraint.
\end{enumerate}
...
\]
Here \(v(x,t)\) is the unknown velocity, \(w(x)\) is an arbitrary admissible
test velocity, \(b(x,t)\) is body force per unit mass, and
\[
        D(v)=\frac12\bigl(\nabla v+(\nabla v)^T\bigr)
\]
is the symmetric strain-rate tensor.  The test space is deliberately
restricted: \(w\) has zero divergence and satisfies the same homogeneous
normal or no-slip boundary condition used by the velocity problem.  The weak
equation must hold for every such \(w\), and the pressure is recovered
afterward as the multiplier that enforces \(\divergence v=0\).
...
\]
The inequality sign is important.  For smooth solutions and boundary
conditions with no boundary power, the corresponding identity is an equality.
For Leray-Hopf weak solutions, the weak limit may lose kinetic energy into
unresolved small scales, so the formulation requires dissipation no smaller
than the viscous term displayed above but does not assert exact equality.
...
hypotheses, while the functional-analytic theory requires additional care.
