...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[blue, fill=white, inner sep=2pt] at (3.25,3.72) {closed shape loop};
...
curvature. Later geometric swimmer models, including work by Kanso, Marsden,
and Rowley, make this connection computation explicit for finite-dimensional
shape families.
...
\(X\), and this \(R\) satisfies \(A=R^{-1}\dd R\).
...
\]
because the \(\dd\omega\wedge\omega\) and \(\omega\wedge\dd\omega\) terms
cancel after expanding. For torsion,
\[
  DT^a
  =
...
\(\delta d_1=\delta\theta\,d_2\) and \(d_1\times n\) is the scalar out-of-plane
moment of the internal force about the centerline.
...
higher nested commutators.} The result is
\[
  g_{\mathrm{loop}}
  =
  \exp\left(
...
The kinetic-energy calculation was done in
Section~\ref{sec:two-rotor-deforming-body-model}. The laboratory uses that
result rather than repeating the algebra:
\[
  \mathcal A_\alpha=
