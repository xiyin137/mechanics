...
A falling cat can reorient in mid-air without any external torque. Its angular
momentum can remain zero while its orientation changes, because the body cycles
through internal shapes. Related geometric-phase mechanisms appear in
...
The word ``gauge'' is used here in a concrete mechanical sense.  The physical
object is the placed deforming body.  To describe it, we choose a convention
for separating overall rigid placement from internal shape.  That convention
is not unique, just as a vector potential is not unique in electromagnetism.
Changing the convention changes the components of the connection, but it does
not change the actual rigid displacement reconstructed after a closed shape
cycle.
...
For readers meeting this language for the first time, the operational test is
simple.  Give the body a closed cycle of shapes \(s(t)\) with \(s(t_1)=s(t_0)\).
If the body returns to the same shape but not to the same orientation or
position, the difference is holonomy.  The connection is the infinitesimal rule
used to reconstruct that rigid motion from the shape velocity; curvature is the
leading small-loop diagnostic for how much holonomy a loop encloses.

\subsection{Principal-Bundle Form Of The Split}
\label{subsec:deforming-body-principal-bundle}

The principal-bundle notation is local but will recur throughout the chapter.
\(Q_{\mathrm{full}}\) is the full configuration space of placed shapes,
\(G\) is the rigid-motion group, \(\mathcal S\) is the shape space, and
\(\pi:Q_{\mathrm{full}}\to\mathcal S\) forgets the rigid placement.  A point
\(q\in Q_{\mathrm{full}}\) is a full configuration, while
\(s=\pi(q)\) is only its shape.
...
\begin{hypothesis}[Regular shape bundle]
...
In this subsection \(A(s)\) denotes a mechanical connection, not the
Laplace-Runge-Lenz vector and not a vector potential in physical space.  The
rigid velocity is \(\xi\in\mathfrak g\), the shape velocity is \(\dot s\), and
\(\Pi\) is the total rigid-motion momentum written in body coordinates.  The
zero-momentum condition determines the rigid velocity required by a prescribed
shape change.
...
\[
  \xi=g^{-1}\dot g\in\mathfrak g,
\]
where \(\mathfrak g\) is the Lie algebra of \(G\). The block form below is just
the kinetic energy written in the split variables \((\xi,\dot s)\).  To see
where it comes from, let \(Y(X,s)\) be the body-frame position of material point
\(X\) at shape \(s\), with the center of mass already separated.  In the
simplest rotational case \(x=RY(X,s)\) and
\[
  R^{-1}\dot x=\Omega\times Y+\pd_\alpha Y\,\dot s^\alpha ,
\]
where \(\Omega\) is the body angular velocity corresponding to \(\xi\).  More
generally, \(\xi\) acts on \(Y\) by the infinitesimal rigid-motion velocity.
Denote that infinitesimal velocity by \(\xi^\#Y\).  Squaring this velocity and
integrating over the body gives three terms:
\[
  \int_B\rho\,|\xi^\#Y|^2\,\dd^3X,\qquad
  2\int_B\rho\,(\xi^\#Y)\cdot(\pd_\alpha Y)\,\dot s^\alpha\,\dd^3X,\qquad
  \int_B\rho\,(\pd_\alpha Y\cdot\pd_\beta Y)
    \dot s^\alpha\dot s^\beta\,\dd^3X .
\]
The first term is the rigid locked inertia \(M(s)\), the third is the shape
metric \(m(s)\), and the cross term is the off-diagonal coupling \(A(s)\).
Thus suppose the kinetic energy has the block form
\[
  T
  =
  \frac12
...
\begin{hypothesis}[Regular locked inertia]
...
In this subsection \(A=R^{-1}\dot R\) is the skew angular-velocity matrix.  It
is distinct from the off-diagonal kinetic-energy block \(A(s)\) in the previous
subsection.  The connection one-form is written calligraphically as
\(\mathcal A\) throughout the chapter.  Here \(Q\) is a second moment tensor of
the instantaneous shape.  The symbol \(\widetilde I\) is the inertia tensor
about the current internal frame, and \(\widetilde J\) is the internal angular
momentum generated by deformation. These tildes distinguish the quantities from
rigid-body \(I\) and \(J\) used earlier.
...
\paragraph{Local notation.}
The map \(h:\mathcal S\to G\) is a change of local section over shape space.
The one-form \(\mathcal A=\mathcal A_\alpha ds^\alpha\) takes values in the Lie
algebra \(\mathfrak g\).  The curvature \(\mathcal F\) below is a two-form on
shape space, and \(\mathcal P\) denotes path ordering in the reconstruction
integral.
...
\]
and the factors are appended chronologically on the right of the current
configuration. Thus the finite product for \(g(T)\) is
\(\exp X_0\exp X_1\cdots\exp X_{N-1}\), with earlier steps written farther to
the left in this reconstruction convention. The factors must remain in this
order because elements of \(G\) need not commute. Taking the mesh size to zero
gives the displayed \(\mathcal P\exp\) formula.
...
\[
  ...
  \mathcal F
  =
  \dd\mathcal A-\frac12[\mathcal A\wedge\mathcal A],
  }
\]
or in components,
\[
...
\]
Here \([\mathcal A\wedge\mathcal A]\) is the bracketed wedge, so its
\((\alpha,\beta)\)-component is
\(2[\mathcal A_\alpha,\mathcal A_\beta]\). This convention removes the common
factor-of-two ambiguity in Lie-algebra-valued curvature formulas. The minus
sign is tied to the minus sign in the reconstruction equation. The local frame
connection used later for Cosserat and defect geometry follows the standard
Maurer-Cartan convention \(F=\dd A+\frac12[A\wedge A]\), with components
...
closed loop detects curvature. The curvature is the local gauge-covariant
diagnostic of the description: under a change of body frame it is conjugated,
not left componentwise unchanged. Different choices of body frame change
\(\mathcal A\), while the holonomy around a loop changes by conjugation and
therefore represents the same physical displacement.
...
\paragraph{Local notation.}
The carrier angular velocity is \(\Omega\), while \(\alpha\) and \(\beta\) are
internal rotor angles.  The unit vectors \(e_x,e_y,e_z\) are body-frame axes.
The constants \(I_0,I_1,I_2\) are moments of inertia of the carrier and
rotors, not action variables.
...
\paragraph{Local notation.}
Here \(u(x,t)\) is the fluid velocity, \(p(x,t)\) is pressure, and \(\mu\) is
fluid dynamic viscosity.  The rigid velocity \(\xi=g^{-1}\dot g\) packages the
swimmer's translational and rotational velocity in a body frame, while
\(\dot s\) is shape velocity.  The matrix \(R(s)\) below is hydrodynamic
resistance, not a rotation matrix.
...
\paragraph{Local notation.}
The coordinate \(X\) labels a material point, \(R(X,t)\) is a local director
frame, \(d_a\) are spatial directors, \(A_A=R^{-1}\partial_A R\) is a material
connection component, and \(A_t=R^{-1}\partial_tR\) is its time component.  The
symbol \(\Gamma_A\) denotes translational strain in the local frame.
...
\paragraph{Local notation.}
In this section \(A_A\) and \(\omega^a{}_b\) are two notations for the same
local frame connection, depending on whether matrix or differential-form
components are being displayed.  \(F_{AB}\) is curvature here, not the
deformation gradient of Chapter~\ref{chap:elastic-body}.  The torsion two-form
\(T^a\) is also unrelated to kinetic energy.
...
Nonzero curvature means that a prescribed connection cannot be written, even on
a contractible patch, as \(R^{-1}\dd R\) for a single smooth orientation field
\(R(X)\). In defect theory,
...
\paragraph{Local notation.}
The coordinate \(s\) is arclength along the rod, not a point of finite
shape space.  The vector \(r(s)\) is the centerline, \(\theta(s)\) the director
angle, \(\kappa=\theta'\) the curvature, \(B\) the bending stiffness, and
\(M=B(\kappa-\kappa_0)\) the bending moment.  A prime denotes
\(\dd/\dd s\) throughout this section.
...
\begin{aligned}
  ...
  \mathcal F=\dd\mathcal A-\frac12[\mathcal A\wedge\mathcal A],\\
  \text{Cosserat continuum:}&\quad R(X,t),\quad R^{-1}\partial_A R,\\
  \text{defect theory:}&\quad A,\quad F=\dd A+\frac12[A\wedge A],\quad
  T=\dd\theta+\omega\wedge\theta.
\end{aligned}
\]
...
\paragraph{Local notation.}
The index \(\alpha=1,2\) labels shape coordinates in this exercise.  The
symbols \(\mathcal A_1,\mathcal A_2\) mean the connection evaluated on the
coordinate directions \(\partial/\partial s^1\) and \(\partial/\partial s^2\).
The group element \(g_{\mathrm{loop}}\) is the net rigid motion after the
closed shape loop.
...
\]
This is why curvature is the local, frame-covariant density of geometric phase.
Even if the connection components are constant in a chosen frame, a nonabelian
structure group can give nonzero holonomy through the commutator term. The
helicopter rotor laboratory below is a concrete instance of this
constant-connection case.

\subsection{Helicopter Rotor Model As A Gauge Connection}
\label{sec:lab-helicopter-gauge}

A finite-dimensional helicopter-rotor model makes the Shapere-Wilczek
connection explicit.
