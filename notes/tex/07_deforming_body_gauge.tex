...
that a connection supplies a chosen horizontal subspace, so the phrase ``pure
shape change'' has a specified meaning. The mechanical connection is the
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[blue, fill=white, inner sep=2pt] at (3.05,3.70) {closed shape loop};
  \begin{scope}[shift={(1.2,1.2)}, scale=0.36]
    \draw[fill=gray!20, thick] (-0.8,0) ellipse (0.45 and 0.25);
    \draw[fill=gray!20, thick] (0.8,0) ellipse (0.45 and 0.25);
    \draw[thick] (-0.35,0) -- (0.35,0);
  \end{scope}
  \begin{scope}[shift={(5.0,1.4)}, scale=0.36, rotate=-20]
    \draw[fill=gray!20, thick] (-1.0,0) ellipse (0.55 and 0.18);
    \draw[fill=gray!20, thick] (1.0,0) ellipse (0.55 and 0.18);
    \draw[thick] (-0.45,0) -- (0.45,0);
  \end{scope}
  \begin{scope}[shift={(5.35,3.05)}, scale=0.36, rotate=45]
    \draw[fill=gray!20, thick] (-0.8,0) ellipse (0.45 and 0.25);
    \draw[fill=gray!20, thick] (0.8,0) ellipse (0.45 and 0.25);
    \draw[thick] (-0.35,0) -- (0.35,0);
...
parameter or shape spaces, but the constructions differ. Berry's phase is a
quantum \(U(1)\) holonomy, Hannay's angle is the classical adiabatic analogue
on invariant tori, and the falling-cat effect is a generally nonabelian
\(\SE(3)\) or \(\SO(3)\) holonomy on shape space with no adiabatic limit
required. The historical anchors are Berry (1984), Hannay (1985), the
Shapere-Wilczek geometric-phase program (1989), and Montgomery's gauge-theory
treatment of the falling cat (1993). The common word is geometric phase, not
...
\(g^{-1}\dot g=-\mathcal A(\dot s)\). The essential content is the holonomy: a
closed loop in internal shape variables produces a rigid rotation even though
the rotors return to their starting angles. Like the falling cat, this is a
nonabelian \(\SO(3)\) holonomy over shape space, although the physical
constraints and shape coordinates are different.
...
curvature is associated with disclinations, or rotational defects; classical
continuum defect theory was developed by Kondo, Bilby, Kroner, and others.
...
two-forms; for example \((*T^a)_i=\frac12\epsilon_i{}^{jk}T^a{}_{jk}\). In this
language:
\[
\begin{aligned}
...
\]
The Cosserat brothers' 1909 theory introduced precisely this idea that a
continuum point can carry an independent local frame, not only a position.
Let
\[
...
Which rigid motion is physical, and which is a frame convention? How much
holonomy is captured by curvature for a small loop, and what changes for a
large loop? How does the finite-dimensional connection become a field theory
over a material body?
...
The kinetic-energy calculation of
Section~\ref{sec:two-rotor-deforming-body-model} gives the same zero-momentum
connection for this laboratory model:
\[
  \mathcal A_\alpha=
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \end{scope}
  \draw[gray!85, thick, ->] (4.30,2.62) --
    (5.18,1.62);
  \node[above, fill=white, inner sep=1pt] at (4.60,2.76)
      {\([\mathcal A_\alpha,\mathcal A_\beta]\)};
