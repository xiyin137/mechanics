...
cycles through internal shapes. The same mechanism appears in low-Reynolds
swimming, astronaut reorientation, and the rotor laboratory below: closed loops
in shape space produce rigid motions in physical space.
...
reference body. Gauge kinematics adds the rigid placement of a shape-changing
body in space. A choice of body axes at each shape gives a local description,
while holonomy around a closed loop records the observable rigid motion produced
by the shape cycle.
...
the rotors return to their starting angles. This is the same geometric-phase
mechanism as the falling cat, written as a connection over shape space.
...
\]
The connection is now computed from a hydrodynamic boundary-value problem. A
reciprocal one-parameter stroke encloses no area in shape space and produces no
net motion: this is Purcell's scallop theorem in gauge language. A
two-parameter stroke can enclose area and swim by curvature.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \end{scope}
  \draw[gray!85, thick, ->] (4.25,1.50) --
    node[above, fill=white, inner sep=1pt]
      {\([\mathcal A_\alpha,\mathcal A_\beta]\)}
    (5.20,1.50);
