...
A Newtonian fluid does not assign stress to a static shear strain relative to a
preferred reference shape. Push a finger through honey: the displaced material
moves, stretches, and eventually fills in behind the finger, but no permanent
shear strain is stored. Pull the finger out and the honey does not snap back.
This is a constitutive idealization, not the definition of every material
called a fluid: viscoelastic fluids, Bingham fluids, granular media, and
superfluids require different constitutive structures.
...
\paragraph{Roadmap.}
The derivation is staged in the order of the modelling choices. First we learn
how to differentiate quantities attached to moving fluid parcels. Then we
convert integral balance laws into local partial differential equations for
mass and momentum. Only after those kinematic and balance statements are in
place do we introduce the Newtonian constitutive law that turns Cauchy's
momentum equation into the Navier-Stokes equation. The later sections use that
equation in three ways: exact laminar solutions, stability analysis, and
Hamiltonian point-vortex reduction.
...
Appendix~\ref{app:continuum-caveats} compares the elastic and fluid versions of
these continuum assumptions, including weak formulations, pressure as a
constraint multiplier, and the difference between smooth derivations and weak
solution theory.

\paragraph{Derivation audit trail.}
The derivation has four ingredients: kinematics, mass balance, momentum
balance, and constitutive law. Each section below pairs an equation with the
conservation statement that produced it. Navier-Stokes appears only after the
Newtonian stress law is imposed, so
...
The coordinate \(x\) is a spatial point and \(X\) is a material label.  The
operator \(D/Dt\) differentiates along the particle path passing through
\(x\) at time \(t\).  It is different from a partial derivative at fixed
spatial position.
...
\paragraph{Local notation.}
The set \(V(t)\) is a moving material volume, while \(U\) in the derivation is
the fixed set of material labels that maps to \(V(t)\).  The deformation
gradient \(F^i{}_A\) and Jacobian \(J=\det F\) are temporary Lagrangian
bookkeeping devices used to prove an Eulerian transport formula.
...
expand or contract. The Reynolds transport theorem is the formula that
keeps both effects.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \draw[->] (-0.4,0) -- (6.8,0) node[above right] {\(x^1\)};
  ...
  \node[blue!70!black, fill=white, inner sep=1pt] at (2.25,1.75) {\(V(t)\)};
  \node[red!75!black, fill=white, inner sep=2pt] at (5.14,3.06)
    {boundary velocity \(u\)};
...
\paragraph{Local notation.}
Here \(\rho(x,t)\) is spatial mass density and \(u(x,t)\) is velocity.  A
material volume is one that moves with the fluid.  Incompressibility is the
condition \(\nabla\cdot u=0\), while constant density is a compatible special
case, not the definition itself.
...
\paragraph{Local notation.}
The stress tensor \(\sigma_{ij}\) is Cauchy stress, force per current area.
The traction \(t(n)=\sigma n\) is the force per area exerted across a surface
whose outward unit normal is \(n\).  The body force \(b\) is force per unit
mass, so \(\rho b\) is force per current volume.
...
There are three levels in this equation.  The left side is the rate of change
of the total momentum of the same moving material volume.  The first term on
the right is a body force distributed through volume, such as gravity.  The
second is the force exerted across the boundary by neighboring fluid or by a
solid wall.  The stress tensor enters only after Cauchy's theorem turns this
surface force into a local linear function of the normal \(n\).
...
The name Navier-Stokes compresses four contributions. Navier (1822) gave a
molecular model for viscosity; Cauchy's stress work from 1822--1827 supplied
the continuum framework; Saint-Venant (1843) added continuum viscous stresses;
and Stokes (1845) gave the formulation that became standard. Truesdell and
Noll later systematized objectivity and constitutive response in the 1960s.

\paragraph{Local notation.}
The pressure \(p\) is the scalar normal stress variable, \(\mu\) is dynamic
shear viscosity, \(\lambda\) is second viscosity, \(D\) is the symmetric
strain-rate tensor, and \(W\) in the velocity-gradient split below is the
antisymmetric spin tensor, not stored energy.

The balance laws so far are universal for continua, but they are not yet a
closed fluid model: the stress tensor \(\sigma\) is still unknown. For a simple
Newtonian fluid the closure is
\[
  \boxed{
  \sigma=-pI+2\mu D+\lambda(\nabla\cdot u)I.
  }
\]
Here \(p\) is pressure, \(\mu\geq0\) is shear viscosity, \(\lambda\) is the
second viscosity coefficient, and
\[
  D_{ij}=\frac12(\partial_i u_j+\partial_j u_i)
\]
is the symmetric strain-rate tensor.

This is the constitutive step.  Momentum balance says how stress accelerates
fluid; the constitutive law says what stress the material produces during a
given motion.  Replacing the law below would give a different continuum theory,
such as an elastic solid, a non-Newtonian fluid, or a micropolar fluid.  Thus
Navier-Stokes is not Newton's law alone: it is Newton's law plus the Newtonian
stress law and the chosen mass-balance constraint.

The assumptions behind the formula are local, linear, objective, and isotropic.
The stress may depend on the instantaneous rate of deformation, not on the
accumulated shear history; this is the point at which fluids separate from
elastic solids. To see why only \(D\) appears, split the velocity gradient into
...
produce viscous stress in an isotropic simple fluid. The constitutive
...
and its trace. The symmetric strain-rate tensor decomposes as
\[
...
rotations. Schur's lemma, in this elementary real representation-theoretic
setting, says that \(L\) acts by a scalar on each irreducible piece. Hence
\(L(D)=a(\tr D)I+bD_0\), which is the displayed law after renaming constants.
...
Here \(\Omega\) is the spatial fluid domain, not an angular velocity.  The
velocity field is \(u(x,t)\), the initial velocity is \(u_0(x)\),
\(\partial\Omega\) is the boundary, \(n\) is the outward unit normal, and
\(U_{\mathrm{wall}}\) is the prescribed wall velocity.  Pressure \(p\) is
defined only up to an additive constant in a closed incompressible domain.
...
ideal equation.
Prandtl's 1904 boundary-layer theory explains how no slip at a wall can coexist
with high-Reynolds-number flow that is nearly inviscid away from the wall: the
tangential adjustment is confined to a thin viscous layer.
...
For three-dimensional finite-energy flows, Leray's 1934 weak solutions provide
the standard global framework. They satisfy the equations in integral form and
obey an energy inequality, while global smooth regularity in three dimensions
...
The dimensional scales in this section are \(U\) for speed and \(L\) for
length.  Primed variables are dimensionless.  The density \(\rho\) and dynamic
viscosity \(\mu\) are assumed constant, and
\(\nu=\mu/\rho\) is the kinematic viscosity.  This \(\mu\) is a fluid
viscosity, not the elastic shear modulus of Chapter~\ref{chap:elastic-body}.
...
\paragraph{Local notation.}
The domain \(\Omega\) is fixed in space in this energy estimate.  The kinetic
energy density is \(\frac12\rho|u|^2\), and \(\mu|\nabla u|^2\) is the
dissipation density after integration by parts under the stated boundary
conditions.
...
Helmholtz (1858) proved the vortex theorems; Kelvin (1869) proved circulation
conservation. Both survive in the inviscid limit and explain which parts of
vorticity motion persist without viscosity.
...
\paragraph{Local notation.}
In this section \(\omega\) is vorticity, not angular velocity and not the
symplectic form.  The circulation of a closed curve \(\gamma(t)\) is
\(\oint_{\gamma(t)}u\cdot\dd x\).  A material loop is transported by the fluid
velocity \(u\).

Taking the curl of the incompressible Navier-Stokes equation, with no body
force or with a conservative body force whose curl vanishes, gives
\[
  \boxed{
...
\]
The identity behind this equation is
\[
  \nabla\times(u\cdot\nabla u)
  =
  ...
  \omega\,\nabla\cdot u,
\]
and the last term vanishes for incompressible flow.
...
For a compressible inviscid barotropic fluid the corresponding identity is
\[
  \frac{D\omega}{Dt}
  =
  \omega\cdot\nabla u-\omega\,\nabla\cdot u.
...
\[
  \boxed{
  \frac{D}{Dt}\left(\frac{\omega}{\rho}\right)
  =
  \left(\frac{\omega}{\rho}\right)\cdot\nabla u.
  }
\]
This is the precise stretching law behind Helmholtz's vortex-line statement.
For \(\nu=0\), vortex lines are transported by the flow. In two-dimensional
incompressible Euler flow this reduces to scalar advection of vorticity; in
three dimensions transport is accompanied by the stretching term.
...
vorticity equation for a barotropic fluid,
\[
  \frac{D}{Dt}\left(\frac{\omega}{\rho}\right)
  =
  \left(\frac{\omega}{\rho}\right)\cdot\nabla u,
...
material line element; hence vortex lines move with the fluid. In the
incompressible case \(\rho\) is constant and this is the usual
...
Bernoulli's \emph{Hydrodynamica} (1738) predates Navier-Stokes by nearly a
century, yet its central invariant follows cleanly from Euler's equation. For
inviscid barotropic flow with conservative body force
\(-\nabla\Phi\),
\[
  \frac{D u}{D t}=-\nabla h-\nabla\Phi,
\]
...
\paragraph{Local notation.}
The potential \(\Phi\) is body-force potential per unit mass, \(h\) is specific
enthalpy, and \(\omega=\nabla\times u\) is vorticity.  Steady means
\(\partial_tu=0\) in the chosen inertial frame.  The Bernoulli constant is
streamline-wise unless the flow is also irrotational on a connected region.
...
Exact Navier-Stokes solutions all exploit one of three mechanisms: a velocity
field that kills \(u\cdot\nabla u\) by directionality, a fully developed
geometry that makes the same cancellation transverse, or a pressure field that
absorbs the nonlinear term as a gradient. The examples below isolate these
mechanisms.

\paragraph{Local notation.}
In this section \(V\) is a scalar velocity profile, not a material volume.
For channel flows \(y\) is wall-normal distance and \(x\) is the streamwise
coordinate.  The pressure gradient \(G=-\dd p/\dd x\) is taken positive when it
drives flow in the positive streamwise direction.

The examples are organized by the mechanism that makes the nonlinear PDE
solvable.  In each example the reader should identify the ansatz, check
incompressibility, compute \(u\cdot\nabla u\), and only then solve the
remaining scalar equation.  The boundary conditions are part of the solution:
they decide which member of the reduced family is being described.
...
\paragraph{Local notation.}
The profiles below use \(V(y)\) or \(V(r)\) for the nonzero velocity component.
The symbol \(Q\) in the flow formulas is volume flux, not a configuration
manifold.  The wall shear \(\tau\) is a stress component, not a time variable.
...
the Navier-Stokes derivation came later. The chronology is a reminder that some
continuum laws entered mechanics through measurement, not from constitutive
postulates.
...
\]
Regularity at the centerline requires \(rV'(r)\to0\) as \(r\to0\). If
\(C_1\neq0\), then \(V'(r)\sim C_1/r\) and the next integration would produce a
logarithmic singularity in \(V\). Hence \(C_1=0\). A second integration gives
\[
...
\centering
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \draw[->] (-0.25,0) -- (-0.25,3.25);
  \node[left, fill=white, inner sep=1pt] at (-0.28,3.18) {\(y\)};
  ...
  \end{scope}
\end{tikzpicture}
\caption{Two steady shear profiles between parallel plates separated by
distance \(h\).  In both panels the vertical direction in the drawing is the
wall-normal coordinate \(y\), with \(y=0\) at the lower wall and \(y=h\) at the
upper wall; the horizontal arrow length represents the streamwise velocity
component \(V(y)\).  Left: Couette flow, \(V(0)=0\) and \(V(h)=U\), is linear
because it is driven by wall motion.  Right: Poiseuille flow,
\(V(0)=V(h)=0\), is parabolic because it balances the pressure force
\(-\partial_xp=G\) against viscous diffusion of momentum.  The arrows are
spatial velocity vectors, not a graph with velocity on the vertical axis.}
...
\paragraph{Local notation.}
Here \(V(y,t)\) is the streamwise velocity induced by the moving wall,
\(\nu=\mu/\rho\) is kinematic viscosity, \(U\) is wall speed, and
\(\eta=y/(2\sqrt{\nu t})\) is a similarity variable.  The function
...
\paragraph{Local notation.}
The frequency \(\Omega_s\) is the imposed wall oscillation frequency; it is not
the spatial fluid domain \(\Omega\) and not angular velocity of a rigid body.
The thickness \(\delta_s=\sqrt{2\nu/\Omega_s}\) is a viscous penetration
depth.
...
\paragraph{Local notation.}
The constant \(k\) is the spatial wavenumber, \(U_0\) is the initial amplitude,
and \(A(t)\) is the decaying amplitude.  The pressure \(p(x,y,t)\) is chosen so
that the nonlinear advection term is exactly balanced by a pressure gradient.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \begin{scope}[yscale=2.0]
    ...
    \node[blue!70!black, fill=white, inner sep=1pt] at (0.72,0.20) {\(t_1\)};
    \node[red!75!black, fill=white, inner sep=1pt] at (1.58,0.48) {\(t_2\)};
    \node[purple!75!black, fill=white, inner sep=1pt] at (2.52,0.22) {\(t_3\)};
  \end{scope}
  \node[align=center, fill=white, inner sep=1.5pt] at (1.8,-0.55)
    {Stokes first problem:\\viscous penetration grows like \(\sqrt{\nu t}\)};

  \begin{scope}[xshift=6.0cm]
    \draw[->] (-0.2,0) -- (3.2,0) node[right] {\(x\)};
    \draw[->] (0,-0.2) -- (0,3.2) node[above] {\(y\)};
    \draw[step=1.0, gray!35, thin] (0,0) grid (3,3);
    \draw[teal!75!black, thick, ->] (0.86,0.50) arc (0:300:0.36);
    \draw[teal!75!black, thick, ->] (1.86,0.50) arc (0:-300:0.36);
    \draw[teal!75!black, thick, ->] (2.86,0.50) arc (0:300:0.36);
    \draw[teal!75!black, thick, ->] (0.86,1.50) arc (0:-300:0.36);
    \draw[teal!75!black, thick, ->] (1.86,1.50) arc (0:300:0.36);
    \draw[teal!75!black, thick, ->] (2.86,1.50) arc (0:-300:0.36);
    \draw[teal!75!black, thick, ->] (0.86,2.50) arc (0:300:0.36);
    \draw[teal!75!black, thick, ->] (1.86,2.50) arc (0:-300:0.36);
    \draw[teal!75!black, thick, ->] (2.86,2.50) arc (0:300:0.36);
    \node[align=center, fill=white, inner sep=1.5pt] at (1.5,-0.55)
      {Taylor-Green vortex:\\periodic eddies decay as \(e^{-2\nu k^2t}\)};
  \end{scope}
\end{tikzpicture}
\caption{Two unsteady exact solutions, drawn by evaluating the formulas in the
text rather than by numerically integrating Navier-Stokes.  Left: an
impulsively moved wall creates the profile
\(V/U=\operatorname{erfc}(y/(2\sqrt{\nu t}))\).  The horizontal axis is the
distance \(y\) from the wall, the vertical axis is the normalized velocity
\(V/U\), and the three curves show increasing times \(t_1<t_2<t_3\); the growing
penetration depth is the viscous scale \(\sqrt{\nu t}\).  Right: the square is
one periodic \(x,y\) cell of the Taylor-Green velocity
\((A\sin kx\cos ky,-A\cos kx\sin ky)\).  The curved arrows indicate the
alternating eddy circulation; advection is absorbed by pressure while viscosity
damps the common amplitude by \(e^{-2\nu k^2t}\).}
...
Local notation separates the base flow from the perturbation.  The scalar
\(U(y)\) is the streamwise base velocity, \(e_x\) is the unit vector in the
streamwise direction, \(P(x)\) is the base pressure, and tildes denote
perturbations: \(\tilde u\) for velocity and \(\tilde p\) for pressure.  In
the Orr-Sommerfeld subsection below, \(D\) means differentiation with respect
to \(y\), not the material derivative.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \draw[red!75!black, densely dashed] (2.75,1.15) -- (3.42,1.15);
  \draw[red!75!black, thick, ->] (2.75,2.05) -- (3.55,2.05);
  \node[red!75!black, fill=white, inner sep=1pt] at (2.43,2.18) {\(\tilde v\)};
  \node[red!75!black, fill=white, inner sep=1pt] at (3.62,0.90) {\(\Delta U\)};
  \draw[purple!75!black, thick, domain=0.25:5.55, samples=180]
    plot (\x,{1.75+0.22*sin(210*\x)});
  \node[blue!70!black, fill=white, inner sep=1pt] at (4.15,2.75) {base shear \(U(y)e_x\)};
  \node[purple!75!black, fill=white, inner sep=1pt] at (3.90,1.32)
...
The Orr-Sommerfeld operator is fourth-order and non-self-adjoint. Orr and
Sommerfeld derived the normal-mode equation independently in 1907--1908. The
equation is linear, but the physical transition problem is not exhausted by
linear eigenvalues: Reynolds' 1883 dye-injection experiment had already shown
that pipe flow can transition to turbulence through finite disturbances.
...
The proof idea is a change of wave-vector variables. A three-dimensional
disturbance proportional to
\[
  e^{i(\alpha x+\beta z)+\sigma t}
\]
has total horizontal wavenumber \(k=(\alpha^2+\beta^2)^{1/2}\). Squire's
transformation maps it to a two-dimensional disturbance with streamwise
wavenumber \(k\), the same phase speed, and effective Reynolds number
\[
  \mathrm{Re}_{2D}=\frac{|\alpha|}{k}\,\mathrm{Re}\le \mathrm{Re}.
\]
In the transformed variables, the wall-normal velocity equation keeps the
Orr-Sommerfeld form with \(\alpha\) replaced by \(k\). The remaining
three-dimensional wall-normal vorticity variable satisfies the Squire equation
and is forced by the two-dimensional velocity mode. Thus any growing
three-dimensional normal mode implies a growing two-dimensional mode at no
larger Reynolds number. The statement is linear and applies to parallel shear
flows; it is not a claim about finite-amplitude transition.
Seek normal modes
\[
...
\(\mathrm{Re}\to\infty\). This is why inviscid inflection-point criteria and
viscous Orr-Sommerfeld thresholds should be compared with care rather than
...
about \(5772\), using centerline speed and half-channel width; Thomas (1953)
established the numerical value, and Orszag (1971) refined it. Plane Couette
...
disturbances. Reynolds' 1883 dye-injection observation is the founding
empirical demonstration. Stability is therefore not a single concept:
...
reveals the only possible energy source for perturbation growth. This identity
is the Reynolds-Orr energy equation for perturbations of a shear flow.
The boundary conditions are part of the identity: the integrations by parts
assume periodic boundaries, no-slip walls, or no-penetration perturbations with
boundary terms that do no work.  The sign of the production term is local in
space.  A perturbation with \(\tilde u\,\tilde v\,U'(y)<0\) extracts kinetic
energy from the mean shear, while the opposite correlation returns energy to
the mean flow.  If \(U'(y)=0\), no mean-shear production exists and the
disturbance energy decays monotonically under viscosity.  Energy stability is
therefore a sufficient stability criterion, not a necessary one: failure of the
estimate means that the energy method has lost control, not that a growing
...
idealizations alongside the 1858 vortex theorems; Kirchhoff cast them as a
Hamiltonian system in his 1876 lectures.
Here they serve as a finite-dimensional Hamiltonian reduction of ideal-fluid
motion.

\paragraph{Local notation.}
The point \(i\) has position \((x_i,y_i)\) and circulation \(\Gamma_i\).  The
Hamiltonian \(H\) is not fluid kinetic energy in full Eulerian form; it is the
finite-dimensional interaction Hamiltonian after point-vortex reduction.  The
two-form \(\omega_{\mathrm{vortex}}\) is symplectic structure, while
\(\omega(x,y)\) above is vorticity.

The point vortex equations are
\[
\begin{aligned}
...
sense. This is consistent with symplectic mechanics: symplectic structure
requires nondegeneracy, not positive definiteness.
...
For completeness, here is the elementary velocity calculation behind the same
Hamiltonian.  A vortex of circulation \(\Gamma_j\) at \(z_j=(x_j,y_j)\)
produces the streamfunction
\[
  \psi_j(x,y)=-\frac{\Gamma_j}{2\pi}
  \log\sqrt{(x-x_j)^2+(y-y_j)^2}.
\]
The velocity field is \(u=(\pd_y\psi,-\pd_x\psi)\).  The singular self-field of
vortex \(i\) is omitted, so
\[
  \dot x_i
  =
  \sum_{j\ne i}\pd_y\psi_j(z_i),
  ...
  -\sum_{j\ne i}\pd_x\psi_j(z_i).
\]
Differentiating the displayed Hamiltonian with respect to \(y_i\) and \(x_i\)
gives exactly these two equations after multiplication by \(\Gamma_i\).  The
sum \(i<j\) counts each interacting pair once; the derivatives see both members
of the pair, which is why no extra factor is missing in Hamilton's equations.
...
\begin{tikzpicture}[scale=1.05, >=Latex, font=\small]
  ...
  \node[fill=white, inner sep=0.2pt] at (-0.65,-0.35) {\(+\)};
  \node[fill=white, inner sep=1pt] (plusgamma) at (-1.55,-0.55) {\(+\Gamma\)};
  \draw[thin] (plusgamma.east) -- (-0.81,-0.38);
  \draw[thick] (0.65,0.35) circle (0.16);
  \node[fill=white, inner sep=0.2pt] at (0.65,0.35) {\(-\)};
  \node[fill=white, inner sep=1pt] (minusgamma) at (1.48,0.78) {\(-\Gamma\)};
  \draw[thin] (minusgamma.west) -- (0.77,0.43);
  \draw[dashed] (-0.65,-0.35) -- (0.65,0.35);
  \draw[<->] (-0.95,-1.02) -- (0.35,-0.32);
  \node[fill=white, inner sep=1pt] at (-0.12,-1.30) {fixed separation \(d\)};
  \draw[blue, thick, ->] (-1.02,-0.62) -- (-1.50,0.26);
  \draw[blue, thick, ->] (0.28,0.08) -- (-0.20,0.96);
  \draw[blue, dashed, thick, ->] (-0.25,1.48) -- (-0.73,2.36);
  \node[blue, align=center, fill=white, inner sep=1pt] at (0.62,1.82)
    {common\\translation};
\end{tikzpicture}
\caption{Point-vortex dipole in the plane.  The displayed axes are spatial
coordinates \(x\) and \(y\).  The two marked singular vortices have equal and
opposite circulations \(+\Gamma\) and \(-\Gamma\), separated by a fixed
distance \(d\).  Each blue arrow is the velocity induced at one vortex by the
other; because the induced velocities are equal and parallel, the pair
translates together without changing separation.  More complicated
point-vortex systems give finite-dimensional Hamiltonian models of
ideal-fluid motion.}
