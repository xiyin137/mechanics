...
A fluid is a continuum that has lost its memory of a reference shear. Push a
finger through honey: the displaced material moves, stretches, and eventually
fills in behind the finger, but no permanent shear strain is stored. Pull the
finger out and the honey does not snap back.

This is why the Eulerian description is natural. We observe fields at fixed
points in space: velocity, density, pressure, and stress. The moving-particle
viewpoint returns through the material derivative and transport theorems. The
preceding elastic and deforming-body chapters kept the material reference
description in the foreground; this chapter shifts the emphasis to Eulerian
fields and treats fluids on their own terms.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \begin{scope}[yscale=2.0]
    ...
    \node[blue!70!black, fill=white, inner sep=1pt] at (1.35,0.13) {\(t_1\)};
    \node[red!75!black, fill=white, inner sep=1pt] at (2.05,0.17) {\(t_2\)};
    \node[purple!75!black, fill=white, inner sep=1pt] at (2.80,0.18) {\(t_3\)};
  \end{scope}
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \draw[red!75!black, thick, ->] (2.75,1.15) -- (2.75,2.05)
    node[midway,right, fill=white, inner sep=1pt] {\(\tilde v\)};
  \draw[red!75!black, thick, ->] (2.75,2.05) -- (3.55,2.05)
    node[right, fill=white, inner sep=1pt] {energy exchange};
