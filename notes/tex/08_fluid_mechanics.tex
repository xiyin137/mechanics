...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[blue!70!black, fill=white, inner sep=1pt] at (2.45,2.0) {\(V(t)\)};
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[red!75!black, fill=white, inner sep=1pt] at (3.48,1.02) {\(\Delta U\)};
...
\]
Thus any growing three-dimensional normal mode implies a growing
two-dimensional mode at no larger Reynolds number. The statement is linear and
applies to parallel shear flows; it is not a claim about finite-amplitude
transition.
Seek normal modes
\[
...
\begin{tikzpicture}[scale=1.05, >=Latex, font=\small]
  ...
  \node[blue, align=center, fill=white, inner sep=1pt] at (1.62,1.22)
    {common\\translation};
\end{tikzpicture}
