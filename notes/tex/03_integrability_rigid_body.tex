...
classical heavy top under its stated assumptions. The rarity of such exceptions
...
The attitude matrix \(R(t)\) maps body components to space components.  The
symbol \(\Omega\) denotes angular velocity written in the moving body frame,
while \(\omega_{\mathrm{space}}\) denotes the same angular velocity written in
fixed space axes.  The hat map sends a vector \(a\in\R^3\) to the skew matrix
\(\widehat a\) satisfying \(\widehat a\,v=a\times v\).
...
The convention in this chapter is that \(R\) maps body components to space
components.  If a material point has fixed body-coordinate vector \(X\), its
space position relative to the center of mass is \(x=RX\).  The columns of
\(R\) are therefore the moving body axes drawn in the fixed space frame.  This
is an active placement convention.  Using the inverse convention would transpose
several formulas and change signs in the body angular velocity; the physical
motion would be the same, but the components would be labelled differently.
...
the same instantaneous rotation.  The matrix \(R^T\dot R\) measures the change
seen by an observer riding with the body, while \(\dot R R^T\) measures the
same change in space axes.  Derivatives of body components must therefore also
...
\paragraph{Local notation.}
The angles \(\phi,\theta,\psi\) are coordinates on a chart of \(\SO(3)\), not
three globally defined functions on all of \(\SO(3)\).  The same letter
\(\theta\) will later be used for action-angle coordinates in an abstract
integrable system; here it is only the Euler polar angle.
...
\paragraph{Local notation.}
Here \(B\) is the material region occupied by the rigid body in body
coordinates, \(X\) labels a material point, and \(\rho(X)\) is mass density.
The inertia tensor \(I_{ij}\) and its principal values \(I_1,I_2,I_3\) are
properties of the mass distribution.  They should not be confused with the
action variables \(I_j\) introduced later in this chapter.
...
\paragraph{Local notation.}
This section temporarily returns to the generic Hamiltonian notation
\((q^i,p_i)\) for coordinates and canonical momenta on \(T^*\SO(3)\).  The
body angular momenta \(M_a\) are fiber coordinates produced by trivializing the
cotangent bundle with the group structure; they are useful, but they are not
canonical momenta conjugate to the coordinates \(q^i\).
...
\paragraph{Local notation.}
The vector \(\Sigma(t)\) is the infinitesimal body-frame rotation used to
describe the variation.  It is not an independent physical angular velocity;
it is a bookkeeping variable for \(\delta R\).  The reduced Lagrangian is
written \(\ell(\Omega)\) below to distinguish it from the full Lagrangian
\(L(R,\dot R)\).
...
\paragraph{Local notation.}
The components \(M_i\) and \(\Omega_i\) are measured in principal body axes.
The scalar \(C=\norm M^2\) introduced below is a Casimir of the reduced
Lie-Poisson bracket; it is not a new energy.  The Hamiltonian \(H\) is kinetic
energy in the free-body problem.
...
\paragraph{Local notation.}
The reconstructed attitude \(R(t)\) is the rotation matrix itself, while
\(M(t)\) is the body-frame angular momentum already obtained from Euler's
equations.  The spatial angular momentum is \(J(t)=R(t)M(t)\).  The symbol
\(\mathcal P\) in the path-ordered exponential is an ordering operation, not a
phase space.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[blue!70!black, align=center, fill=white, inner sep=2pt] at (1.24,0.74)
    {reduced\\Euler curve};
  ...
  \node[red!75!black, align=center, fill=white, inner sep=2pt] at (2.82,2.56)
    {orientation\\fiber};
  \draw[red!75!black, ->] (2.40,2.42) -- (1.44,2.20);
...
\paragraph{Local notation.}
The space called \(M\)-space is \(\mathfrak{so}(3)^*\cong\R^3\) with coordinates
\((M_1,M_2,M_3)\).  The curve drawn in this section is a reduced trajectory;
it is not yet the full attitude motion on \(T^*\SO(3)\).
...
\begin{tikzpicture}[
  ...
  \node[purple!80!black, align=center, fill=white, inner sep=1.5pt] at (-3.95,1.45,0.72)
    {back branch\\(dashed)};
  \draw[purple!80!black, ->] (-3.20,1.25,0.66) -- (-1.05,0.95,0.55);
\end{tikzpicture}
...
\paragraph{Local notation.}
The vector \(\Omega\) is the body angular velocity, while
\(\omega_{\mathrm{space}}=R\Omega\) is its spatial representation.  The
invariable plane \(\Pi_J\) is fixed in space because \(J=RM\) is conserved.
The polhode and herpolhode are curves traced by angular velocity, not by a
material point of the body.
...
\paragraph{Local notation.}
The perturbations \(\alpha_i(t)\) and \(\eta_i\) below are small components
near a principal-axis rotation.  They are not Euler angles.  The ordering
\(I_1<I_2<I_3\) fixes which axis is called intermediate; reversing labels
would relabel the conclusion.
...
\paragraph{Local notation.}
The functions \(F\) and \(G\) are observables of the reduced angular momentum
\(M\).  The gradient \(\nabla F\) is the ordinary Euclidean gradient after
identifying \(\mathfrak{so}(3)^*\) with \(\R^3\).  This reduced bracket differs
from the canonical bracket on \(T^*\SO(3)\).
...
\label{sec:heavy-symmetric-top}

The free rigid body is the cleanest introduction to reduction, but the first
non-free rigid-body example should still be integrable.  The Lagrange heavy top
has a fixed point, a symmetry axis, and gravity.  Assume
\[
  I_1=I_2,\qquad I_3\ \text{arbitrary},
\]
and assume that the center of mass lies a distance \(\ell\) along the body
axis \(e_3\) from the fixed point.  Let \(\theta\) be the angle between \(e_3\)
and the upward vertical.  With the Euler-angle convention of
Section~\ref{sec:euler-angles-so3}, the angular velocity gives
\[
  T
  =
  \frac12 I_1(\dot\theta^2+\dot\phi^2\sin^2\theta)
  +
  \frac12 I_3(\dot\psi+\dot\phi\cos\theta)^2,
\]
...
\[
  L=T-V.
\]
The symbols in this example are local to the heavy top.  The constants
\(I_1=I_2\) and \(I_3\) are principal moments of inertia, \(m\) is the mass,
\(g\) is gravitational acceleration, and \(\ell\) is the distance from the
fixed point to the center of mass.  The angles \(\phi,\theta,\psi\) are Euler
angles; \(p_\phi\) and \(p_\psi\) are their conjugate momenta.  Later in the
chapter \(I\) will also denote action variables, so the subscripted inertia
notation \(I_1,I_2,I_3\) should be kept separate from actions.

The coordinates \(\phi\) and \(\psi\) are cyclic.  Their conjugate momenta are
\[
...
\[
  p_\phi
  =
  I_1\dot\phi\sin^2\theta+p_\psi\cos\theta .
\]
The first is the angular momentum along the body symmetry axis; the second is
the angular momentum along the fixed vertical.  Both are conserved because the
system is invariant under rotations about the body symmetry axis and about the
vertical axis.

Solving for the angular rates gives
\[
  \dot\phi
  =
  ...
  \dot\psi
  =
  \frac{p_\psi}{I_3}-\dot\phi\cos\theta .
\]
The energy becomes a one-dimensional problem:
\[
  E
  =
  \frac{p_\theta^2}{2I_1}
...
\[
  ...
  U_{\rm eff}(\theta)
  =
  \frac{(p_\phi-p_\psi\cos\theta)^2}{2I_1\sin^2\theta}
  +
  \frac{p_\psi^2}{2I_3}
  +
  mg\ell\cos\theta .
...
\]
The nutation angle \(\theta\) oscillates between turning points determined by
\[
  E=U_{\rm eff}(\theta).
\]
Once \(\theta(t)\) is known, \(\phi(t)\) and \(\psi(t)\) follow by quadrature
from the two rate formulas above.  The heavy symmetric top is therefore
integrable by the same logic as a particle in a one-dimensional effective
potential, but its reconstruction still winds on the rotation group.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex,font=\small]
  \draw[->] (-0.2,0) -- (3.55,0) node[right] {\(\theta\)};
  \draw[->] (0,-0.15) -- (0,3.4) node[above] {\(U_{\rm eff}\)};
  \draw[blue!70!black,thick]
    plot[smooth] coordinates {
      (0.18,3.08) (0.35,2.10) (0.58,1.34) (0.86,0.80)
      (1.20,0.56) (1.55,0.63) (1.90,0.93) (2.24,1.45)
      (2.58,2.20) (2.88,2.95) (3.05,3.22)
    };
  \draw[red!75!black,dashed,thick] (0.28,1.35) -- (3.12,1.35)
    node[right] {\(E\)};
  \draw[gray!70,dashed] (0.58,0) -- (0.58,1.35);
  \draw[gray!70,dashed] (2.20,0) -- (2.20,1.35);
  \draw[<->,purple!80!black,thick] (0.58,0.28) -- (2.20,0.28)
    node[midway,below=3pt] {nutation interval};
  \node[blue!70!black,fill=white,inner sep=2pt] at (1.90,2.75)
    {effective potential};
\end{tikzpicture}
\caption{The heavy symmetric top reduces to one-dimensional motion in
\(U_{\rm eff}(\theta)\) at fixed \(p_\phi\) and \(p_\psi\).  The precession and
spin angles are reconstructed after the nutation has been solved.}
\label{fig:heavy-top-effective-potential}
\end{figure}

The same system has a Lie-Poisson form that makes the bridge to geometric
mechanics explicit.  Let
\[
  \Gamma=R^T\hat z
\]
be the upward vertical written in body components, and let
\(\chi=e_3\) be the body symmetry axis.  Then
\[
  H(M,\Gamma)
  =
  \frac12\left(\frac{M_1^2+M_2^2}{I_1}+\frac{M_3^2}{I_3}\right)
  +
  mg\ell\,\Gamma\cdot\chi .
\]
The equations are
\[
  \dot M=M\times\Omega+mg\ell\,\Gamma\times\chi,
  \qquad
...
\[
  H,\qquad
  \norm{\Gamma}^2=1,\qquad
  M\cdot\Gamma=p_\phi,\qquad
  M\cdot\chi=p_\psi .
\]
The last conservation law uses \(I_1=I_2\) and the fact that \(\chi\) is the
symmetry axis.  Without axial symmetry, \(M\cdot\chi\) would not be conserved,
and the effective-potential reduction would disappear.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{../../figures/canonical/heavy_symmetric_top_quick.png}
\caption{Numerical heavy-top laboratory generated by
\protect\path{demos/python/heavy_symmetric_top.py}.  The left panel has horizontal
axis \(\theta\), the polar angle between the body symmetry axis and the
vertical, and vertical axis \(U_{\rm eff}(\theta)\) in the nondimensional
units of the script.  The conserved momenta \(p_\phi\) and \(p_\psi\) are held
fixed; the dashed horizontal line is the conserved reduced energy, and the
vertical marker is the initial value of \(\theta\).  Intersections of the
energy line with \(U_{\rm eff}\) are the nutation turning points; the green
dotted vertical lines mark those angles.  The right panel uses the same reduced
model: its horizontal axis is nondimensional time and its vertical axis is the
numerically integrated angle \(\theta(t)\).  The green dotted horizontal lines
are the same turning-point angles, now drawn as reference levels for
\(\theta(t)\).  Thus the two panels are not independent experiments; they are
the potential picture and the corresponding time trace for one reduced
heavy-top problem.}
\label{fig:heavy-top-numerical-lab}
\end{figure}

\section{Liouville Integrability And Invariant Tori}
\label{sec:liouville-tori}

The notation changes slightly in this section.  The phase space is denoted
\(P\), its dimension is \(2n\), the commuting integrals are
\(F_1=H,F_2,\ldots,F_n\), and \(M_f\) denotes their common level set.  The
letter \(I\) will later denote action variables; it is not the inertia tensor
of the rigid body in this section.
...
followed for all flow times. Dropping any one of these hypotheses changes the
conclusion.
...
\begin{hypothesis}[Regular compact level set]
...
The regularity hypothesis says that \(M_f\) is a smooth \(n\)-dimensional
...
same argument applies at every point of \(M_f\), so each \(\R^n\)-orbit is an
open subset of \(M_f\). The complement of any orbit is a union of other open
orbits. Since \(M_f\) is connected, there can be only one orbit: the commuting
flows act transitively on the level-set component under discussion.

The stabilizer of \(x_0\) consists of the flow times that return \(x_0\) to
itself. It is a discrete subgroup \(\Lambda\subset\R^n\), called the period
lattice, and the only global identifications are translations
\[
  s\sim s+\lambda,\qquad \lambda\in\Lambda,
\]
by elements of this lattice. Thus the transitive orbit identifies
\[
  M_f\simeq \R^n/\Lambda .
\]
Compactness of \(M_f\) now forces \(\Lambda\) to have rank \(n\). If
\(\Lambda\) had rank \(k<n\), then
...
\[
  p_i=p_i(q,F),
\]
then the matrix \(\partial F_k/\partial p_j\) is invertible on the chart being
used.  This is a local rank condition, not an additional integrability
hypothesis: it merely says that this particular canonical chart projects the
torus regularly to the \(q\)-coordinates. On such a chart,
\[
\begin{aligned}
  \omega
  &=
...
\paragraph{Local notation.}
The \(I_j\) in this section are action variables.  They label invariant tori
and are unrelated to the principal moments of inertia \(I_1,I_2,I_3\) used in
the rigid-body sections.  The cycles \(\gamma_j\) form a basis of the first
homology of the torus, so changing that basis changes the numerical action
coordinates by an integral linear transformation.  The orientation of each
cycle fixes the sign of the corresponding action.  In a one-degree-of-freedom
factor, we orient the closed orbit as the positive boundary of the region it
encloses in the \((q,p)\) plane, using the area form \(\dd q\wedge \dd p\);
with this convention \(I=(2\pi)^{-1}\oint p\,\dd q\) is positive for bounded
oscillations and tends to zero as the enclosed region shrinks to a stable
equilibrium.
...
regularity hypotheses, the \(I_j\) can replace the original integrals \(F_j\)
...
\paragraph{Local notation.}
In this one-degree-of-freedom discussion \(q\) and \(p\) are canonical
coordinates, \(E\) is the fixed energy, and \(q_-(E),q_+(E)\) are the turning
points where \(p=0\).  The symbol \(\omega(E)\) at the end of the derivation is
a frequency, not the symplectic form.
...
\begin{tikzpicture}[scale=1.0, >=Latex]
  ...
  \node[align=center, fill=white, inner sep=1pt] at (3.2,0.55)
    {signed phase-space area\\\(I=(2\pi)^{-1}\oint p\,\dd q\)};
\end{tikzpicture}
...
\paragraph{Local notation.}
Here \(J\) is spatial angular momentum and \(J_3\) is its component along a
chosen fixed space axis.  The body angular momentum is \(M=R^TJ\).  The
three-torus is a regular common level set in the full phase space, while the
closed intersection curve in \(M\)-space is its reduced shadow.
...
\item The heavy symmetric top keeps enough symmetry to remain integrable:
gravity breaks full rotational symmetry but leaves two cyclic momenta, reducing
nutation to one-dimensional motion in an effective potential.
...
\[
  ...
  p_\psi^2.
\]
This identity follows directly from the same Euler-angle convention used for
the heavy top.  The body components of angular momentum are
\[
\begin{aligned}
  M_1
  &=
  p_\theta\cos\psi
  +
  \frac{p_\phi-p_\psi\cos\theta}{\sin\theta}\sin\psi,\\
  M_2
  &=
  -p_\theta\sin\psi
  +
  \frac{p_\phi-p_\psi\cos\theta}{\sin\theta}\cos\psi,\\
  M_3&=p_\psi .
\end{aligned}
\]
The dependence on the spin angle \(\psi\) is only a rotation in the
\((M_1,M_2)\)-plane.  Therefore the cross terms cancel in
\(J^2=M_1^2+M_2^2+M_3^2\), leaving the displayed formula.
...
\(\Omega_2\). Let
\[
  x_1=\frac{AI_3-B}{I_2(I_3-I_2)},
  \qquad
  x_3=\frac{B-AI_1}{I_2(I_2-I_1)}.
\]
The root \(x_1\) is where \(\Omega_1=0\), and \(x_3\) is where
\(\Omega_3=0\). In a physical chamber one must also require
\(\Omega_1^2,\Omega_3^2\ge0\). Thus, for the chamber in which the
intermediate-axis component changes sign,
\[
  0\le x=\Omega_2^2\le a^2,
  \qquad
  a^2=\min\{x_1,x_3\},
  \qquad
  b^2=\max\{x_1,x_3\}.
...
\[
  ...
  (a^2-\Omega_2^2)(b^2-\Omega_2^2).
\]
Hence, after choosing the origin of time in this chamber,
\[
  \boxed{
  \Omega_2(t)=a\,\operatorname{sn}(\nu(t-t_0),k),
  }
  \qquad
  ...
  k^2=\frac{a^2}{b^2}.
\]
To check the \(\operatorname{sn}\) form directly, use
\[
  \frac{\dd}{\dd u}\operatorname{sn}(u,k)
  =
  \operatorname{cn}(u,k)\operatorname{dn}(u,k),
\]
together with
\[
  a^2-a^2\operatorname{sn}^2=a^2\operatorname{cn}^2,
  \qquad
  b^2-a^2\operatorname{sn}^2=b^2\operatorname{dn}^2.
\]
Then \(\dot\Omega_2^2\) equals the quartic above precisely when
\[
  \nu=b\sqrt{\frac{(I_3-I_2)(I_2-I_1)}{I_1I_3}}.
\]
The \(\operatorname{dn}\) factor appears instead in the principal-axis
component that does not change sign in the chosen chamber: if \(a^2=x_1\), then
\(\Omega_1\) is proportional to \(\operatorname{cn}\) and \(\Omega_3\) is
proportional to \(\operatorname{dn}\); if \(a^2=x_3\), the roles of
\(\Omega_1\) and \(\Omega_3\) are interchanged. This chamber accounting is the
elliptic-function version of the intermediate-axis instability: the
intermediate component naturally passes through zero, while the dominant
principal-axis component keeps its sign.
...
The script \path{demos/python/heavy_symmetric_top.py} complements this by
integrating the Lagrange top's effective one-dimensional nutation problem and
reconstructing the cyclic Euler angles.  It is the most direct numerical check
that the heavy symmetric top is integrable for a different reason than the
free asymmetric top: axial symmetry supplies the extra conserved momentum.
Use Figure~\ref{fig:heavy-top-numerical-lab} as the diagnostic checklist: the
turning points in the potential plot, the extrema of \(\theta(t)\), and the
reported energy drift must agree before the precession and spin reconstruction
are trusted.
