...
brackets become the rigid-body Lie-Poisson brackets; the variables \(M_a\) are
not canonical coordinates with \(\Poisson{M_a}{M_b}=0\).
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[red!75!black, align=center, fill=white, inner sep=2pt] at (6.45,2.35)
...
Dropping any one of these assumptions changes the conclusion.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[purple!75!black, align=center, fill=white, inner sep=2pt] at (1.45,2.45)
    {Hamiltonian flow\\has constant slope};
  \draw[purple!75!black, ->] (2.30,2.28) -- (2.85,2.24);
\end{tikzpicture}
...
involve elliptic functions in place of the elementary chamber formula above.
