...
angular-momentum sphere. The rigid body is the first place
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[red!75!black, align=center, fill=white, inner sep=2pt] at (5.15,2.82)
...
Completeness is used exactly here: without complete flows, this action would
...
replaced by another local branch or another canonical chart.
...
commuting flows are independent and compact; the exceptional sets are exactly
...
allowed annulus has height \(J-\max\{|a|,|b|\}\), so the action is
\[
  \boxed{
...
the origin of time,
\[
  \Omega_2(t)=a\,\operatorname{cn}(\nu(t-t_0),k)
\]
or an equivalent \(\operatorname{sn}/\operatorname{dn}\) parametrization
