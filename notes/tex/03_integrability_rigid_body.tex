...
the middle axis and the book turns over. This is not a numerical accident. It
is the geometry of two conserved quantities, energy and angular momentum,
intersecting on the angular-momentum sphere. The rigid body is the first place
where the pictures in these notes stop being optional: reduction, stability,
...
variation on the rotation group.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \draw[->] (-0.2,0) -- (3.25,0) node[right] {\(M_1\)};
  \draw[->] (0,-0.25) -- (0,3.25) node[above] {\(M_2\)};
  \draw[blue!70!black, thick]
    (1.35,1.55) ellipse (1.20 and 0.58);
  \node[blue!70!black, align=center, fill=white, inner sep=2pt] at (1.35,2.55)
  ...
  \node[red!75!black, align=center, fill=white, inner sep=2pt] at (4.65,2.75)
  ...
  \begin{scope}[xshift=4.1cm]
    \draw[thick] (0,0.65) rectangle (1.65,2.45);
    \draw[->, purple!75!black, thick]
      (0.18,0.92) .. controls (0.55,1.55) and (1.05,1.72) .. (1.42,2.25);
    \node[purple!75!black, align=center, fill=white, inner sep=2pt] at (0.86,0.20)
...
\begin{tikzpicture}[
  ...
  \node[purple!80!black, fill=white, inner sep=2pt] at (3.45,-1.10)
    {intersection trajectories};
  \draw[purple!80!black, ->] (2.80,-0.90) -- (1.05,0.25);
\end{tikzpicture}
...
velocity space. Define the inertia ellipsoid
\[
...
angular velocity. Thus the free rigid body can be pictured as the inertia
ellipsoid rolling without slipping on the fixed invariable plane.
...
Poinsot's construction is not a separate solution method from the reduced
equations. It is the same conservation of \(H\) and \(J\), viewed in angular
velocity space rather than angular momentum space. Its value is conceptual: it
...
\]
The minus sign matches the body-frame convention used above. Had we used
spatial angular momentum components as reduced variables, the sign would be
opposite. This is another place where the body/space distinction is not
cosmetic: it changes the reduced Poisson bracket while leaving the physical
motion unchanged.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[purple!75!black, align=center, fill=white, inner sep=2pt] at (5.35,2.35)
    {Hamiltonian flow\\has constant slope};
  \draw[purple!75!black, ->] (4.65,2.17) -- (3.35,2.58);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.0, >=Latex]
  ...
  \node[below=6pt] at (1.0,0) {\(q_-\)};
  \node[below=6pt] at (5.3,0) {\(q_+\)};
  \draw[blue!70!black, thick, ->]
...
Evaluating this standard angular-momentum integral gives the piecewise-linear
answer
\[
  \boxed{
...
degeneracy is a symmetry, not chaos. In the nonsymmetric free rigid body the
generic invariant sets are still Liouville tori, but the action integrals
involve elliptic functions rather than the elementary chamber formula above.
