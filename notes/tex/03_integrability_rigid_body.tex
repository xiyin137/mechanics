...
nonintegrability and chaos. The classical integrable exceptions beyond the free
body include Lagrange's symmetric heavy top and Kovalevskaya's 1888 top; their
rarity is part of the lesson.
...
The rigid body supplies the concrete model in which the theorem becomes
visible.
...
form in 1901; the modern symplectic-reduction setting is part of the
Marsden-Weinstein framework.
...
Euler derived these equations in his 1765 theory of rigid-body motion; the
modern reduced variational form is the Euler-Poincare version of the same
calculation.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[red!75!black, align=center, fill=white, inner sep=2pt] at (5.00,2.82)
    {attitude fiber\\over a reduced state};
  \draw[red!75!black, ->, thick] (2.55,1.75) -- (4.0,2.10);
  \draw[red!75!black, ->, thick] (2.45,1.35) -- (4.0,1.05);
...
\begin{tikzpicture}[
  ...
  \node[purple!80!black, fill=white, inner sep=2pt] at (4.45,-3.05,-0.55)
    {intersection trajectories};
  \draw[purple!80!black, ->] (3.70,-2.68,-0.45) -- (1.15,-0.25,0);
\end{tikzpicture}
...
velocity space. Poinsot introduced this rolling picture in 1834; his names
polhode and herpolhode are still the standard names for the body-space and
space-space traces. Define the inertia ellipsoid
\[
...
\begin{tikzpicture}[scale=1.0, >=Latex]
  ...
  \fill[purple!75!black] (0.18,0.55) circle (1.5pt)
      node[below right] {\(\Omega\)};
    \draw[->, red!75!black, thick] (0.18,0.55) -- (0.95,1.25);
...
intermediate axis.
This instability is the mathematical core of the tennis-racket theorem, and of
the Dzhanibekov effect observed in orbit in 1985; the underlying explanation is
already present in Euler's rigid-body equations.
...
The Casimir is why the reduced phase space is foliated by angular-momentum
spheres. Each sphere carries a genuine two-dimensional symplectic structure,
and the rigid body restricted to one such sphere has one degree of freedom.
...
Historically, Liouville proved the analytic integration-by-quadratures part in
1855; the modern torus formulation is usually called the Liouville-Arnold
theorem, after Arnold's 1963 geometric statement.
...
after a linear change of basis
\[
...
than a picture. Enough commuting flows on a compact regular level set force the
level set to be a torus. The angle variables introduced below are coordinates
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[purple!75!black, align=center, fill=white, inner sep=2pt] at (4.85,2.72)
    {Hamiltonian flow\\has constant slope};
  \draw[purple!75!black, ->] (4.20,2.55) -- (3.20,2.48);
\end{tikzpicture}
...
coordinate-free argument above, but the displayed function \(p(q,F)\) must be
replaced by another local branch or another canonical chart. The torus itself
does not become singular at a caustic; only this particular projection to
configuration coordinates does.
...
The rigid-body chapter supplies both a model and a method. The model is the
free body, whose motion is visible through energy ellipsoids, momentum
spheres, Poinsot rolling, and attitude reconstruction. The method is more
general: use symmetry to reduce, use conserved quantities to find invariant
sets, and then ask whether those invariant sets are tori carrying linear
motion.
...
tori.
\item Action variables are period integrals of the canonical one-form. Their
geometric definition explains why the coordinates are adapted to invariant
tori.
\item The operation \(R^T\dd R\) that defines rigid-body angular velocity is
the finite-dimensional prototype of the connection forms used later for
deformable bodies and Cosserat continua.
\end{itemize}

\paragraph{Questions to carry forward.}
Which conserved quantities commute, and which merely coexist? What happens to a
Liouville torus when a resonance is introduced? How much of rigid-body
geometry survives when the body can change shape?
...
For the symmetric top, whose classical heavy-top version was analyzed by
Lagrange in the eighteenth century,
\[
  H
  =
  \frac{J^2-M_3^2}{2I_1}
...
Kirillov symplectic form on the radius-\(J\) angular-momentum sphere has the
property that a cap of height \(h\) has symplectic area \(2\pi h\). The two
fixed projections remove caps determined by \(|a|\) and \(|b|\); the remaining
...
\]
It does not depend on the inertial direction of the angular momentum. This
...
above. Concretely, for \(I_1<I_2<I_3\) in a fixed energy chamber, the Euler
equations reduce to
\[
  \dot\Omega_2^2
  =
...
\]
for constants \(a,b\) determined by \(H\) and \(J^2\). Hence, after choosing
...
depending on the chamber. The elementary symmetric-top formula is therefore a
