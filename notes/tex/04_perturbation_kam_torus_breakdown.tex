...
mechanics. Add Jupiter, and resonances appear. Invariant tori fragment.
Transport channels open. The full restricted three-body formulation, including
...
splitting, and transport. The standard map is this chapter's compact
laboratory; the asteroid problem is the gravitational setting we revisit once
the perturbative mechanisms are clear.
...
Numerical asteroid ejection probabilities are meaningful because the analytic
and geometric levels explain where transport channels come from. The analytic
level asks whether a formal canonical transformation can remove angle
dependence. The geometric level asks which invariant tori, islands, and
separatrices organize phase space. The numerical level asks how an ensemble
moves through that geometry over a finite time.
...
\begin{tikzpicture}[x=0.72cm,y=1cm, >=Latex, font=\small]
  ...
  \node[blue!60!black, fill=white, inner sep=1.5pt] at (3.6,-0.48)
...
Kolmogorov-Arnold-Moser work of the 1950s and early 1960s, which explained why
...
\[
  H(I,\theta)=H_0(I)+\eps H_1(I,\theta)
\]
be a sufficiently smooth, analytic in the standard theorem, nearly integrable
Hamiltonian on \(U\times\mathbb T^N\). Assume \(H_0\) is Kolmogorov
nondegenerate. For
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \draw[thick, blue!70!black] (0.4,0) -- (6.65,0);
  \foreach \x/\w in {
    1.40/0.18,3.70/0.18,5.60/0.18,
    0.92/0.09,2.10/0.08,2.78/0.07,4.35/0.10,4.92/0.08,6.16/0.08,
    0.58/0.045,1.10/0.040,1.78/0.045,2.42/0.040,3.22/0.045,
    4.05/0.040,4.66/0.035,5.25/0.040,5.96/0.040,6.45/0.035}
  {
    \draw[white, line width=5pt] (\x-\w,0) -- (\x+\w,0);
    \draw[red!70!black, very thick] (\x-\w,0.15) -- (\x+\w,-0.15);
    \draw[red!70!black, very thick] (\x-\w,-0.15) -- (\x+\w,0.15);
  }
...
neighboring resonances. Before overlap, islands may be surrounded by surviving
invariant curves. After overlap, hyperbolic manifolds from different resonances
intersect and produce a connected chaotic sea.
...
This theorem is the precise mathematical reason a rational torus generically turns
...
treatments appear in Birkhoff's original work and in modern accounts such as
Moser-Zehnder.
...
When an irrational invariant curve finally breaks, Aubry-Mather theory supplies
the replacement object. For an area-preserving twist map and an irrational
...
criterion. For a rational periodic orbit of period \(q\), the residue of the
\(q\)-fold derivative is
\[
  R=\frac{2-\tr(DT_K^q)}{4}.
\]
For an elliptic periodic point \(0<R<1\); for a hyperbolic point \(R<0\) or
\(R>1\). Greene's residue conjecture says that an irrational invariant curve
...
in the standard map, and direct numerical simulations support it.
...
Which tori are protected by arithmetic, and which are exposed by resonance?
How does a cantorus slow transport after a smooth barrier has broken? Which
features in a finite simulation are KAM geometry, and which are numerical
artifacts?
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[red!75!black, align=center, fill=white, inner sep=1pt] at (0.05,-1.50)
    {three escape\\valleys};
  \draw[red!75!black, ->] (0.58,-1.32) -- (0.08,-0.98);
...
sketch records the phase-space transition from invariant curves to islands and
chaotic scatter. Both panels identify the structures used in the derivation.}
...
islands and chaotic layers.  They are intentionally small so that the phase
portrait can be compared directly with the perturbative derivation.
