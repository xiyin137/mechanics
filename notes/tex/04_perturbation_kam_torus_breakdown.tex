...
The chronology is useful because several ideas are often conflated. Kolmogorov
announced the KAM mechanism in 1954. The Fermi-Pasta-Ulam-Tsingou computations
were run in 1953--1955 and published in 1955 as a striking challenge to naive
ergodicity; the recurrence they observed is one finite-lattice manifestation of
near-integrable dynamics, not itself the KAM theorem. Siegel's 1942 work on
small divisors is an important predecessor. Moser's \(C^k\) theorem in 1962 and
Arnold's analytic proof in 1963 made the survival mechanism into the modern KAM
theorem, with later refinements including P\"oschel's 1982 invariant-torus
theorems and Eliasson's 1988 normal-form work. These developments are parallel
windows onto the same fact: small perturbations can leave most invariant tori
intact while still allowing chaotic regions.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[blue!70!black, align=center, fill=white, inner sep=2pt] at (1.70,0.88)
    {Diophantine\\surviving labels};
  \node[red!70!black, align=center, fill=white, inner sep=2pt] at (5.55,0.94)
...
\begin{tikzpicture}[scale=0.95, >=Latex, font=\small]
  ...
  \begin{scope}[shift={(8.0,0)}]
    ...
    \foreach \x/\y in {0.72/1.02,1.46/1.08,2.33/1.05}
...
\begin{axis}[
  ...
  \node[anchor=west, black!55, fill=white, inner sep=1pt] at (axis cs:5.00,2.53)
    {\(p=2\pi/3\)};
\end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  \begin{scope}
    ...
    \foreach \p in {(0,1.75),(-1.55,-0.95),(1.55,-0.95)}
      \fill[red!75!black] \p circle (1.3pt);
    \draw[red!75!black, ->] (0,1.75) -- (0,2.10);
    \draw[red!75!black, ->] (-1.55,-0.95) -- (-1.90,-1.18);
    \draw[red!75!black, ->] (1.55,-0.95) -- (1.90,-1.18);
    \node[red!75!black, align=center, fill=white, inner sep=1pt] at (0.00,-1.55)
  ...
  \begin{scope}[xshift=5.2cm]
    ...
    \foreach \x/\y in {-1.3/0.9,-1.1/-0.8,-0.4/1.15,0.25/-1.2,1.0/0.75,1.25/-0.55,
      ...
      \fill[black!55] (\x,\y) circle (0.9pt);
    \node[anchor=west, fill=white, inner sep=1pt] at (0.22,1.58)
