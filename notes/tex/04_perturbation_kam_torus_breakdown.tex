...
by writing enough first integrals. Poincare's 1890 negative theorem showed
that generic perturbations of an integrable Hamiltonian need not admit
additional analytic first integrals beyond the energy; his later work on the
three-body problem exposed the stable-unstable manifold tangles behind this
failure. The modern story is more nuanced than ``small perturbations make
chaos.'' Kolmogorov, Arnold, and Moser found the survival theorem hidden inside
the failure: sufficiently irrational invariant tori persist, while resonant
regions become the sites of islands, separatrix splitting, and transport. The
standard map is this chapter's compact laboratory; the asteroid problem is the
gravitational setting we revisit once the perturbative mechanisms are clear.
...
mechanisms concrete in two-dimensional return maps. These mechanisms become the
interpretive language for the asteroid ejection experiment developed in the
three-body chapter.
...
Historically, the missing-asteroid bands at these resonances are the Kirkwood
gaps, identified empirically by Daniel Kirkwood in 1866 before modern
Hamiltonian perturbation theory supplied the phase-space explanation.
...
The historical landmarks should be kept in the right order. Kolmogorov
announced the KAM mechanism in 1954, while the Fermi-Pasta-Ulam-Tsingou
computations were run in 1953--1955 and published in 1955 as a striking
challenge to naive ergodicity. Arnold's analytic proof in 1963 and Moser's
\(C^k\) theorem in 1962 made the survival mechanism into the modern KAM
theorem. These developments are parallel windows onto the same fact: small
perturbations can leave most invariant tori intact while still allowing
chaotic regions.
...
method. In a toy one-mode problem
\[
...
\(F^q\) has at least \(2q\) fixed points in the resonant annulus, equivalently
at least two \(q\)-periodic orbits of \(F\). Generically these periodic points
alternate between elliptic and hyperbolic type around the annulus.
\end{proposition}
...
irrationals in the precise Hurwitz sense: every irrational \(x\) has
...
\]
then the map used above is \(D\circ Kc\), while the drift-kick convention is
\(Kc\circ D=D^{-1}(D\circ Kc)D\) up to the same choice of section along the
kicked flow. At \(K=0\), the invariant curve
\[
...
\begin{axis}[
  ...
  \node[anchor=west, black!55, fill=white, inner sep=1pt] at (axis cs:5.08,2.34)
    {\(p=2\pi/3\)};
\end{axis}
\end{tikzpicture}
...
modes \(k\) and \(k'\) in \(H_1\) can therefore produce \(k+k'=(1,-1)\) or
\((2,-2)\), and these satisfy \((k+k')\cdot\omega_0=0\). The obstruction is not
that the first-order cubic term is already resonant; it is that the
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \node[red!75!black, align=center, fill=white, inner sep=1pt] at (0.05,-1.68)
    {three escape\\valleys};
  \draw[red!75!black, ->] (0.54,-1.48) -- (0.08,-0.98);
  ...
  \begin{scope}[xshift=5.2cm]
    ...
    \foreach \x/\y in {-1.3/0.9,-1.1/-0.8,-0.4/1.15,0.25/-1.2,1.0/0.75,1.25/-0.55,
      -1.45/0.25,-0.95/1.25,-0.75/-1.18,-0.15/0.95,0.45/-0.85,0.75/1.12,
      1.38/0.18,1.52/-0.92,-1.55/-0.42}
      \fill[black!55] (\x,\y) circle (0.9pt);
...
a schematic chaotic layer. Both panels identify the structures used in the
derivation.}
