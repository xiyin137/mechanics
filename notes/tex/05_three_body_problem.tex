...
Two bodies under mutual gravity move on conic sections: ellipses, parabolas, or
hyperbolas, with collision as a singular limiting case. Three bodies do not
admit a comparable solution in elementary functions. Sundman gave a convergent
series solution in 1912, away from triple collision, but that result does not
restore practical integrability or the Kepler conic picture. Binary collisions
can be regularized by changing variables and time scale. In the planar problem,
the Levi-Civita 1903 change \(z=u^2\), together with a time rescaling
\(\dd t=|u|^2\,\dd s\), turns the inverse-square collision singularity into a
regular passage in the \(u\)-plane; Sundman's construction uses this kind of
regularization together with analytic continuation and a global time
reparametrization. Sundman announced the key regularization program in 1907 and
published the convergent-series theorem in 1912. Triple collision is the
obstruction left out of Sundman's
global statement; in the three-body problem it is tied to the exceptional
zero-angular-momentum collapse channel.
Belorizky already emphasized in 1930 that Sundman's series converges far too
slowly for practical computation. Adding one more gravitating body preserves
the standard conservation laws of Newtonian mechanics but destroys generic
integrability. What remains is a rich set of structures:
rotating-triangle exact solutions, equilibrium points in the rotating frame,
resonant islands, chaotic scattering, and long-time transport.
The historical drama matters because it calibrates the claim: Poincare's work
on the King Oscar problem exposed homoclinic tangles as a mechanism by which a
deterministic Hamiltonian system can evade integration by additional first
integrals.
The prize essay was submitted for King Oscar II's 1889 competition and then
substantially revised after Poincare discovered the error that led him to
homoclinic intersections; the expanded account appeared in the
\emph{Methodes nouvelles de la mecanique celeste} (1892--1899).
Birkhoff's 1915 periodic-orbit work continued this program by making return
maps and periodic orbits central tools in celestial mechanics.
Painleve's 1895 singularity theorem clarifies a separate issue: in the
Newtonian three-body problem, every finite-time singularity is caused by a
collision. Xia's 1992 noncollision singularities for five bodies show that this
is a special feature of low \(N\), not a general safety theorem for Newtonian
gravity.
...
Before specializing to a massless asteroid, it is useful to state the full
three-body problem. This prevents a common misunderstanding: the restricted
problem is not a different force law, but a controlled limit and coordinate
choice inside the same Newtonian interaction. The full problem also explains
why the two-body Kepler miracle is exceptional. Translation, rotation, and
energy still give conserved quantities, but they no longer reduce the dynamics
to a one-dimensional radial equation.

Let \(q_i(t)\in\R^3\) be the inertial position of mass
\(m_i>0\), and write
\[
  r_{ij}=q_i-q_j,\qquad r_{ij}=\norm{q_i-q_j}.
\]
The Lagrangian is
\[
  L(q,\dot q)
...
\begin{remark}[Why the two-body miracle fails]
...
Lagrange found the equilateral family in his 1772 study of the three-body
problem. The collinear central configurations below go back to Euler's 1767
work and are the finite-mass ancestors of the collinear Lagrange points in the
restricted problem.
...
of \(x\) gives the displayed quintic. The algebra is lengthy but has no
new idea: \(x\) is the only shape variable, while \(d\) sets the overall scale.
\end{example}
...
Much later, Chenciner and Montgomery found the figure-eight periodic orbit
(2000), a striking equal-mass solution with zero angular momentum. It is not a
central configuration, but pedagogically it reinforces the same lesson: special
exact orbits can be beautiful without making the generic three-body problem
integrable.
...
mass, so it feels the primaries but does not affect them. Historically, this is
the setting of Jacobi's integral (1836) and Hill's lunar restricted problem
(1878). After choosing units
in which
\[
  G(m_1+m_2)=1,\qquad
...
\]
All four eigenvalues are purely imaginary exactly when the two roots in
\(\lambda^2\) are real and negative, which is equivalent to
\[
  1-27\mu(1-\mu)\ge0.
\]
...
This is why Trojan motion near the triangular points is a natural classroom
example of stable motion in a nontrivial three-body setting.
The stability criterion is historically due to Gascheau (1843) and Routh
(1875). Its Solar System relevance became visible with the discovery of Trojan
asteroids such as 588 Achilles near Jupiter's triangular points in 1906.
\end{remark}
...
\begin{definition}[Ejection criterion in the demo]
...
An ejection probability is a finite-time statistic tied to a specified
...
Which structures come from exact Newtonian symmetries, and which come from the
restricted approximation? How does the Jacobi constant shape escape channels?
Which ejection statistics remain stable when the timestep, ensemble, or
planetary mass is varied?
...
\begin{tikzpicture}[scale=1.15, >=Latex, font=\small]
  ...
  \fill[blue!70!black] (r1) circle (2pt)
    node[right, fill=white, inner sep=1pt] {\(r_1\)};
