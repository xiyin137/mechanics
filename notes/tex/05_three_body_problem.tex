...
\begin{center}
\small
\begin{tabular}{@{}p{0.20\textwidth} p{0.23\textwidth} p{0.25\textwidth} p{0.17\textwidth}@{}}
\hline
...
by secular torques.  A binary-single encounter is less nearly integrable during
close approach, but its outcomes are still classified by the binding energies
of the outgoing pairs.
...
calculation.  Consider an inner Kepler pair with semimajor axis \(a_1\),
...
The Hamiltonian may be checked against the vector form
\[
  \Phi_{\rm quad}
  \propto
...
pipeline.  For seeds \(s\in S\), the code estimates
\[
...
\(v_\infty\).  For each speed it samples binary phase and impact parameter,
classifies finite-horizon outcomes by pair binding energies, and reports
\[
...
\]
for uniform-in-area sampling.  The expected qualitative trend is that lower
incoming speeds have larger capture-or-exchange cross sections, because
gravitational focusing and longer interaction times make temporary binding
easier.  The benchmark is still a classroom calculation: capture means
...
finite-time stability, capture, exchange, or ejection observable.
\item The Lidov--Kozai mechanism is the controlled hierarchical-triple example:
double averaging leaves one slow degree of freedom, conserves
...
not universal constants, but ensemble-dependent scattering observables. Use the
Lidov--Kozai demo for the integrable secular hierarchical-triple limit, and use
the benchmark driver when the goal is an aggregate statistical result rather
than a single smoke-test trajectory.
