...
The three-body problem is the first Newtonian gravitational system in which all
of the themes of these notes meet: variational mechanics, Hamiltonian phase
space, symmetry reduction, integrable limits, resonances, and chaos. The
two-body problem is exactly solvable because its symmetries and hidden
conserved quantities reduce the motion to conic sections. Adding one more
gravitating body preserves the obvious conservation laws but destroys generic
integrability. The resulting system contains special exact solutions, stable
and unstable relative equilibria, resonant islands, chaotic scattering, and
long-time transport.

This chapter is written as a self-contained bridge between the Kepler problem
and the asteroid-belt simulations in the demos. We first formulate the full
Newtonian problem and reduce translations. We then describe Jacobi coordinates,
angular momentum, and the special central configurations. The circular
restricted three-body problem is derived as a controlled limit in which two
massive primaries move on circular Kepler orbits and the third body is a test
particle. Finally we connect that model to Sun-Jupiter-asteroid dynamics and to
numerical estimates of ejection probability.

\paragraph{Roadmap.}
The chapter deliberately moves through three models rather than jumping
straight to a simulation. The full three-body problem supplies the Hamiltonian,
the conserved quantities, and the reason generic integrability fails. The
circular restricted three-body problem supplies the rotating frame, Jacobi
constant, zero-velocity curves, and Lagrange points. The heliocentric
Sun-Jupiter-asteroid model supplies the finite-time ensemble experiment. Each
model keeps part of the previous one and discards part of it; the text names
those decisions so that numerical ejection is not mistaken for an intrinsic
property of an unspecified ``three-body problem.''
...
\end{array}
\]
The later levels are not replacements for the earlier ones. They are controlled
reductions chosen to isolate transport, escape channels, and resonance-driven
loss in a teachable setting.
...
The center-of-mass degree of freedom is dynamically trivial, and it is worth
removing it explicitly rather than merely saying ``work in the center-of-mass
frame.''  Let
\[
...
\[
  ...
  -U(\rho_1,\rho_2),
\]
where \(\pi_a=\mu_a\dot\rho_a\).  This formula is the cleanest way to see what
...
\]
With \(H=T-U\), we obtain
\[
  \boxed{\ddot I_{\mathrm{cm}}=4H+2U.}
\]
This identity is not a solution formula, but it is a powerful diagnostic.  For
a bounded noncollisional motion, the long-time average of \(\ddot I_{\mathrm{cm}}\)
vanishes and one obtains the virial relation \(2\langle T\rangle=\langle U\rangle\).
...
radial problem, and the Laplace-Runge-Lenz vector gives the conic orbit. For
three bodies there is no single relative radius and no central force after
reduction. Pairwise forces keep changing the effective center seen by each
body, so the Kepler constants are no longer constants of the full motion.
\end{remark}
...
The circular restricted three-body problem is the cleanest autonomous model
...
mass, so it feels the primaries but does not affect them. After choosing units
in which
\[
  G(m_1+m_2)=1,\qquad
...
\]
The Coriolis force is velocity dependent but does no work; it bends trajectories
on the energy surface without changing the Jacobi constant below.
\end{derivation}
...
\begin{tikzpicture}[scale=1.25, >=Latex]
  ...
  \node[yellow!45!black, below left=2pt] at (pone) {\(P_1\)};
  \fill[orange!80!black] (ptwo) circle (2.6pt);
  \node[orange!80!black, below right=2pt] at (ptwo) {\(P_2\)};
  \foreach \pt/\lab/\anchor in {lone/L_1/above left,ltwo/L_2/above,lthree/L_3/above}
  {
  ...
  \draw[blue!60!black, ->] (1.55,1.00) arc (28:78:0.78);
  \node[blue!60!black, fill=white, inner sep=1pt] at (1.95,1.48) {rotating frame};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.0, >=Latex]
  \begin{scope}
    \fill[gray!15] (-2.0,-1.15) rectangle (2.0,1.15);
    ...
    \draw[red!75!black, thick] (0.2,-0.08) -- (0.2,0.08);
    \node[red!75!black, above] at (0.2,0.08) {\(L_1\)};
  ...
  \begin{scope}[xshift=5.6cm]
    ...
    \draw[blue!70!black, thick]
      (-1.35,0) .. controls (-1.20,0.70) and (-0.30,0.78) .. (0.20,0.20)
      .. controls (0.70,0.60) and (1.25,0.45) .. (1.35,0)
      .. controls (1.25,-0.45) and (0.70,-0.60) .. (0.20,-0.20)
      .. controls (-0.30,-0.78) and (-1.20,-0.70) .. (-1.35,0);
    \fill[white]
      (-1.22,0) .. controls (-1.08,0.58) and (-0.30,0.62) .. (0.20,0.10)
      .. controls (0.64,0.48) and (1.08,0.36) .. (1.18,0)
      .. controls (1.08,-0.36) and (0.64,-0.48) .. (0.20,-0.10)
      .. controls (-0.30,-0.62) and (-1.08,-0.58) .. (-1.22,0);
    \fill[yellow!80!black] (-0.75,0) circle (2.8pt);
    \fill[orange!80!black] (0.88,0) circle (2.2pt);
    \draw[red!75!black, thick] (0.2,-0.08) -- (0.2,0.08);
    \node[red!75!black, above] at (0.2,0.08) {\(L_1\)};
...
\(\phi=n_Jt\) and its conjugate action, but the resulting system has more than
one degree of freedom and can exhibit resonance and chaotic transport.
...
\]
These labels should not be overinterpreted: a bin is not a resonance island,
and the real resonance width depends on eccentricity and perturbation strength.
They are a diagnostic overlay, useful for asking whether large loss fractions
cluster near the same places where perturbation theory predicts slow resonant
angles.
...
Hamiltonian, and the Jacobi constant. These structures are the cleanest
...
\begin{tikzpicture}[scale=1.15, >=Latex, font=\small]
  ...
  \node[red!75!black, align=center, fill=white, inner sep=1.5pt] at (1.65,1.24)
