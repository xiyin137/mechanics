...
\(C=F^TF\), are the natural first objects.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \begin{scope}[xshift=8.6cm]
    \draw[->] (-1.35,0) -- (1.55,0) node[right] {\(e_1\)};
    \draw[->] (0,-1.15) -- (0,1.35) node[above] {\(e_2\)};
    \draw[red!70!black, very thick, ->] (0.78,-0.05) arc (-5:30:0.78);
...
condition. Changing the initial \(\xi(X_0)\) adds a translation, and changing
\(\omega(X_0)\) adds an infinitesimal rotation. Thus the only freedom left by
the compatible strain is the expected rigid motion.
...
of propagating plane waves requires \(c_P^2>0\) and \(c_S^2>0\), namely
\(\mu>0\) and \(\lambda+2\mu>0\). This is strictly weaker than positive
definiteness of the quadratic elastic energy, which gives
\(\mu>0\) and \(K_{\mathrm{bulk}}=\lambda+\frac23\mu>0\). Wave-speed positivity
therefore does not by itself imply the stronger energy convexity condition.
In finite elasticity these distinctions become sharper. Positive wave speeds
are related to strong ellipticity, or positivity of the second variation on
rank-one perturbations \(a\otimes n\). Energy minimization requires stronger
conditions, often expressed through rank-one convexity, quasiconvexity, or the
more checkable sufficient condition of polyconvexity. These are not cosmetic
terms: a stored energy can be frame indifferent and support real small-amplitude
waves while still failing to be globally convex as a function of all entries of
\(F\).
The \(P\)- and \(S\)-wave names are not only formal: seismology uses precisely
this longitudinal/transverse split, and Oldham's 1906 inference of Earth's core
from the \(S\)-wave shadow zone is a memorable physical consequence of the same
calculation.
...
Saint-Venant's principle is the philosophical license behind many of the
one-dimensional reductions below. Details of how a load is applied at an end are
expected to decay over distances comparable to the cross-section size, so a
slender body can have a robust interior bar, shaft, or beam solution even when
the end fixtures are complicated. This is an asymptotic modelling principle, not
an exact theorem for every geometry.
...
\]
This static Navier-Cauchy equation descends from Navier's 1821 molecular theory
and Cauchy's continuum formulation of stress in the 1820s.
...
admissible displacement space. Korn's inequality is the missing analytic
bridge:
\[
  \|\xi\|_{H^1(B)}
...
by the symmetric part once rigid rotations are excluded. With Korn's inequality,
...
The Euler-Bernoulli model traces back to Daniel Bernoulli's elastic-energy
viewpoint and Euler's 1744 elastica. Its assumptions are deliberate: later
...
Which tensor lives in the reference configuration, and which lives in space?
Which compatibility conditions distinguish an actual deformation from a strain
field with defects? Which boundary term defines the stress being measured?
