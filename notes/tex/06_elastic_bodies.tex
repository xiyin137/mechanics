...
The elastic case is the cleaner first continuum model because every material
point keeps a persistent label.  This lets us carry the configuration-space
logic of the finite-dimensional chapters almost unchanged into field theory.
This chapter therefore begins with the material map
\[
...
then treated after these material-body ideas are in place.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \begin{scope}[xshift=8.6cm]
    ...
    \node[red!70!black, fill=white, inner sep=1pt] at (1.30,0.84) {\(R_{\mathrm p}\)};
...
\(-e_i\) and areas \(A_i=n_iA\). Force balance on the tetrahedron gives
\[
...
Therefore \(t(n)=\sum_i n_i t(e_i)\). Define \(\sigma\) by taking its \(i\)-th
column to be \(t(e_i)\). Then \(t(n)=\sigma n\), and uniqueness follows by
...
admissible displacement space. Korn's inequality controls the full displacement
gradient by the strain, and the Lax-Milgram theorem then gives a unique weak
