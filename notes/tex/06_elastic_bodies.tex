...
terms scale like volume, so \(O(V)/A\to0\). Hence
\[
  t(n)=-\sum_i n_i t(-e_i).
\]
...
\]
and, after fixing rigid motions or imposing displacement boundary conditions,
the \(L^2\)-term can be removed. The inequality is an integration-by-parts
statement that the skew part of \(\nabla\xi\) is controlled by the symmetric
part once rigid rotations are excluded. With Korn's inequality, the bilinear
elastic energy is coercive, and the Lax-Milgram theorem gives a unique weak
solution for compatible loads. In a pure traction problem, uniqueness holds
only modulo rigid motions, which must be removed by side constraints.
...
For a non-circular cross-section \(A\), one writes the axial displacement as a
warping function \(\chi(x,y)\) and solves a two-dimensional Poisson problem on
\(A\) with Neumann boundary condition determined by the boundary tangent. The
torsional rigidity is then not \(\mu J_{\mathrm{polar}}\), but an energy
integral involving \(\chi\). The circular formula is the special case in which
symmetry forces \(\chi=0\).
