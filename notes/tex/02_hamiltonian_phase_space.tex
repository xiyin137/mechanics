...
\[
  ...
  \end{pmatrix}.
\]
Darboux's proof can be sketched as a local induction. Choose a function
...
\end{array}
\]
Each line is just a different Legendre transform of the same exact one-form.
...
The rest of the notes repeatedly reuse three lessons from this chapter.
\begin{itemize}
\item The configuration space \(Q\) is part of the model, and boundary terms
are information: they identify canonical momenta, forces, and conserved
quantities.
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \fill[red!75!black] (0.95,-0.57) circle (1.3pt);
  \node[black, fill=white, inner sep=1pt] at (-1.38,1.72) {\(W=0\)};
  \node[black, fill=white, inner sep=1pt] at (1.38,-1.72) {\(W=0\)};
