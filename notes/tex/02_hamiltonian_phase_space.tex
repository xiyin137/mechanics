...
\[
  \Theta=p_i\,\dd q^i.
\]
The terminology remembers Liouville's nineteenth-century role in the invariant
volume and integration theory of Hamiltonian systems; the object itself is the
...
carries a closed nondegenerate two-form of dimension \(2n-2\). Repeating the
...
This coordinate-free language became standard in the twentieth-century
geometric formulation of mechanics, with Lichnerowicz, Kirillov, and Souriau
among the central figures. The formula is not an additional physical postulate;
it is Hamilton's equations written without choosing canonical coordinates.
If
\[
...
\[
  ...
  0.
\]
The type-II generating function is this local potential.
Impose
\[
...
of the method. The pendulum, standard map, and asteroid demos are therefore
experiments in preserving structure.
This viewpoint is the basis of symplectic integration, developed in modern
numerical form by Ruth's 1983 explicit symplectic schemes, the work of
Channell-Scovel and Sanz-Serna, Yoshida's 1990 composition methods, and the
backward-error viewpoint later systematized by Hairer, Lubich, and Wanner.
...
Which transformations preserve \(\omega\) itself? When does the Legendre
transform fail? Which numerical errors change the geometry of phase space, and
which ones only change the parametrized trajectory?
...
\begin{tikzpicture}[scale=1.0, >=Latex, font=\small]
  ...
  \fill[red!75!black] (-0.95,0.57) circle (1.3pt)
    node[above left, fill=white, inner sep=1pt] {\(W=0\)};
  \fill[red!75!black] (0.95,-0.57) circle (1.3pt)
    node[below right, fill=white, inner sep=1pt] {\(W=0\)};
  \draw[decorate, decoration={brace, amplitude=5pt}]
    (1.45,-0.57) -- (1.45,0.57);
  \draw[->, gray!70] (2.52,0.45) -- (1.50,0.28);
  \node[align=left, fill=white, inner sep=1.5pt] at (2.66,0.88)
    {multiple \(\dot x\)\\for one \(p\)};
