...
\(q^1\) with nonzero differential. Nondegeneracy gives a Hamiltonian vector
field transverse to its level sets; integrate that vector field to construct a
conjugate coordinate \(p_1\) with \(\{q^1,p_1\}=1\). The symplectic orthogonal
to the span of \(\partial_{q^1},\partial_{p_1}\) carries a closed nondegenerate
two-form of dimension \(2n-2\), so the construction repeats. The result is a
coordinate chart in which \(\omega=\dd q^i\wedge\dd p_i\).
...
below. Once \(\omega\) is given, the bracket is the corresponding algebraic
...
numerical form by Ruth, Channell-Scovel, Sanz-Serna, Yoshida, and others in the
1980s and 1990s.
